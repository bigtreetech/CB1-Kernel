				 ...
				 enum p2p_group_removal_reason removal_reason)
{
	...
	if (removal_reason != P2P_GROUP_REMOVAL_SILENT) {
		...
		asp_on_p2p_group_removed(&wpa_s->asp,
					 os_memcmp(gtype, "GO", 2) == 0,
					 ssid->ssid, ssid->ssid_len, reason);
#endif
	}
...
#ifdef CONFIG_ASP
	asp_on_p2p_group_started(&wpa_s->asp, go, ssid_txt,
				 freq, psk_txt, passphrase,
				 go_dev_addr, persistent, extra);
#endif /* CONFIG_ASP */
}
...
#ifdef CONFIG_ASP
	asp_on_group_formation_completed(&wpa_s->asp, success);
#endif
...
#ifdef CONFIG_ASP
	asp_on_go_neg_completed(&wpa_s->asp, res);
#endif
...
#ifdef CONFIG_ASP
	asp_on_go_neg_req_rx(&wpa_s->asp, src, dev_passwd_id, go_intent);
#endif
}
...
#ifdef CONFIG_ASP
			asp_on_search_result(&wpa_s->asp,
					     info->p2p_device_addr,
					     info->device_name,
					     id, str, info->p2ps_instance);
#endif
		}
		goto done;
	}
...
#ifdef CONFIG_ASP
static void wpas_prov_disc_req_sent(void *ctx, int success)
{
	struct wpa_supplicant *wpa_s = ctx;
	asp_on_prov_disc_req_sent(&wpa_s->asp, success);
}

static void wpas_go_neg_req_sent(void *ctx, int success)
{
	struct wpa_supplicant *wpa_s = ctx;
	asp_on_go_neg_req_sent(&wpa_s->asp, success);
}

static void wpas_p2ps_prov_disc_req_recvd(void *ctx, const u8 *dev,
					  const char *dev_name,
					  ...
					  size_t session_info_len)
{
	...
	asp_p2ps_prov_disc_req_rx(&wpa_s->asp, dev, dev_name, follow_on,
				  status, adv_mac, ses_mac, adv_id, ses_id,
				  conncap, auto_accept, session_info,
				  session_info_len);
}
#endif


static void wpas_p2ps_prov_complete(void *ctx, const struct  p2ps_provision *p2ps_prov,
				    ...
				    const u8 *group_ssid, size_t group_ssid_len)
{
		...
		if (!go_wpa_s) {
			...
			if (!go_ifname[0]) {
				wpas_p2ps_prov_complete(
					wpa_s, p2ps_prov, P2P_SC_FAIL_UNKNOWN_GROUP,
					dev, adv_mac, ses_mac,
	...
	} else if (conncap == P2PS_SETUP_NEW && p2ps_prov) {
		wpa_msg_global(wpa_s, MSG_INFO,
			       P2P_EVENT_P2PS_PROVISION_DONE MACSTR
			       " status=%d conncap=%x"
			       " adv_id=%x adv_mac=" MACSTR
			       ...
			       p2ps_prov->pd_seeker);
#ifdef CONFIG_ASP
		asp_on_pd_complete(&wpa_s->asp, p2ps_prov, status, dev, adv_mac,
				   ses_mac, grp_mac, adv_id, ses_id,
				   conncap, passwd_id, persist_ssid,
				   persist_ssid_size, response_done,
				   prov_start, session_info, feat_cap,
				   feat_cap_len, freq, group_ssid,
				   group_ssid_len);
#endif
	} else {
		wpa_msg_global(wpa_s, MSG_INFO,
...
#ifdef CONFIG_ASP
	p2p.prov_disc_req_sent = wpas_prov_disc_req_sent;
	p2p.go_neg_req_sent = wpas_go_neg_req_sent;
	p2p.p2ps_prov_disc_req_recvd = wpas_p2ps_prov_disc_req_recvd;
#endif
		     ...
		     bool allow_6ghz)
{
	...
	if (auth) {
		wpa_printf(MSG_INFO, "authorizing go neg\n");
				  ...
				  bool allow_6ghz)
{
	...
	if (ssid->disabled != 2 || ssid->ssid == NULL || wpa_s->global->p2p ==NULL)
		return -1;
