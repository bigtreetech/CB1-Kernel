...
static void wpas_trigger_scan_cb(struct wpa_radio_work *work, int deinit)
{
	...
	if (ret) {
		...
		wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_SCAN_FAILED "ret=%d%s, global retry=%d",
			ret, retry ? " retry=1" : "", wpa_s->wpa_scan_retry);
		radio_work_done(work);

		if (retry && wpa_s->wpa_scan_retry < 3) {
			/* Restore scan_req since we will try to scan again */
			wpa_s->scan_req = wpa_s->last_scan_req;
			wpa_supplicant_req_scan(wpa_s, 1, 0);
			wpa_s->wpa_scan_retry++;
		} else if (wpa_s->scan_res_handler) {
			/* Clear the scan_res_handler */
			wpa_s->scan_res_handler = NULL;
		}

		if(wpa_s->wpa_scan_retry >= 3)
			wpa_s->wpa_scan_retry = 0;
