...
#ifdef CONFIG_ASP
static int p2p_ctrl_asp_seek(struct wpa_supplicant *wpa_s, char *cmd,
			     char *buf, size_t buflen)
{
	u32 search_id;

	if (asp_seek(wpa_s, &search_id) < 0)
		return -1;

	if (p2p_ctrl_find(wpa_s, cmd) < 0)
		return -1;

	os_snprintf(buf, buflen, "ASP_SEEK search_id=%u", search_id);
	return os_strlen(buf);
}


static int p2p_ctrl_asp_advertise(struct wpa_supplicant *wpa_s, char *cmd,
				  char *buf, size_t buflen)
{
	char *pos;
	char *adv_str;
	...
	*pos++ = '\0';
	...
	if(asp_advertise(wpa_s, auto_accept, adv_str,
			(u8) svc_state, (u16) config_methods,
			svc_info, cpt_prio, &adv_id) < 0)
		return -1;

	os_snprintf(buf, buflen, "ASP_ADVERTISE adv_id=%u", adv_id);
	return os_strlen(buf);
}


static int p2p_ctrl_asp_cancel_advertise(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	u32 adv_id;
	...
	return asp_cancel_advertise(wpa_s, adv_id);
}


static int p2p_ctrl_asp_service_status_change(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	u32 adv_id;
	...
	/* if (pos == NULL) */
	/* 	return -1; */
	/* status = strtol(pos + 7, NULL, 10); */

	if (!wpas_p2p_service_p2ps_id_exists(wpa_s, adv_id))
		return -1;

	/* TODO */
	/* return asp_service_status_change(wpa_s, adv_id, status); */
	return 0;
}


static int p2p_ctrl_asp_cancel_seek(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	u32 search_id;

	pos = os_strstr(cmd, "search_id=");
	if (!pos || sscanf(pos + 10, "%llx", &val) != 1 || val > 0xffffffffULL)
		return -1;
	search_id = val;

	return asp_cancel_seek(wpa_s, search_id);
}

static int p2p_ctrl_asp_connect_session(struct wpa_supplicant *wpa_s, char *cmd,
					char *buf, size_t buflen)
{
	u8 addr[ETH_ALEN];
	struct p2ps_provision *p2ps_prov;
	...
	os_snprintf(aug_cmd, sizeof(aug_cmd),
		    "%s session=%u session_mac="MACSTR,
		    cmd, 0, MAC2STR(wpa_s->global->p2p_dev_addr));

	wpa_printf(MSG_INFO, "%s: %s", __func__, aug_cmd);
	...
	if (asp_connect_session(wpa_s, addr, NULL, p2ps_prov, &session_id) < 0)
		return -1;

	os_snprintf(buf, buflen,
		    "ASP_CONNECT_SESSION session_mac="MACSTR" session_id=%u",
		    MAC2STR(p2ps_prov->session_mac), session_id);
	return os_strlen(buf);
}

static int p2p_ctrl_asp_remove_session(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	unsigned int uival;
	...
	/*
	 * <addr> session_mac=<addr> session_id=<session_id> reason=<reason>
	 */
	...
	pos = os_strstr(cmd, "session_id=");
	if (!pos || sscanf(pos + 11, "%llx", &val) != 1 || val > 0xffffffffULL)
		return -1;
	session_id = val;

	pos = os_strstr(cmd, "reason=");
	if (!pos || sscanf(pos + 7, "%u", &uival) != 1)
		return -1;
	if (0xFF <= uival)
		return -1;
	reason = (u8)uival;

	return asp_remove_session(wpa_s, addr, session_mac, session_id, reason);
}

static int p2p_ctrl_asp_bound_port(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	unsigned int uival;
	...
	/*
	 * <session_mac> <session_id> <port> <proto>
	 */
	...
	/* session mac */
	pos = cmd;
	if (hwaddr_aton(pos, session_mac))
		return -1;
	pos += 18;

	/* session_id (hex) */
	if (sscanf(pos, "%llx", &val) != 1 || val > 0xffffffffULL)
		return -1;
	session_id = val;
	...
	return asp_bound_port(wpa_s, session_mac, session_id, port, proto);
}

static int p2p_ctrl_asp_release_port(struct wpa_supplicant *wpa_s, char *cmd)
{
	char *pos;
	long long unsigned val;
	unsigned int uival;
	...
	return asp_release_port(wpa_s, session_mac, session_id, port, proto);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		wpas_p2p_stop_find(wpa_s);
#ifdef CONFIG_ASP
	} else if (os_strncmp(buf, "ASP_SEEK ", 9) == 0) {
		reply_len = p2p_ctrl_asp_seek(wpa_s, buf + 8, reply,
					      reply_size);
	} else if (os_strncmp(buf, "ASP_ADVERTISE ", 14) == 0) {
		reply_len = p2p_ctrl_asp_advertise(wpa_s, buf + 14, reply,
						   reply_size);
	} else if (os_strncmp(buf, "ASP_SERVICE_STATUS_CHANGE ", 26) == 0) {
		reply_len = p2p_ctrl_asp_service_status_change(wpa_s,
							       buf + 26);
	} else if (os_strncmp(buf, "ASP_CANCEL_ADVERTISE ", 21) == 0) {
		if (p2p_ctrl_asp_cancel_advertise(wpa_s, buf + 21))
			reply_len = -1;
	} else if (os_strncmp(buf, "ASP_CANCEL_SEEK ", 16) == 0) {
		if (p2p_ctrl_asp_cancel_seek(wpa_s, buf + 16))
			reply_len = -1;
	} else if (os_strncmp(buf, "ASP_CONNECT_SESSION ", 20) == 0) {
		reply_len = p2p_ctrl_asp_connect_session(wpa_s, buf + 20,
							  reply, reply_size);
	} else if (os_strncmp(buf, "ASP_REMOVE_SESSION ", 19) == 0) {
		if (p2p_ctrl_asp_remove_session(wpa_s, buf + 19))
			reply_len = -1;
	} else if (os_strncmp(buf, "ASP_BOUND_PORT ", 15) == 0) {
		if (p2p_ctrl_asp_bound_port(wpa_s, buf + 15))
			reply_len = -1;
	} else if (os_strncmp(buf, "ASP_RELEASE_PORT ", 17) == 0) {
		if (p2p_ctrl_asp_release_port(wpa_s, buf + 17))
			reply_len = -1;
#endif
