...
static int wpa_cli_cmd_asp_provision(struct wpa_ctrl *ctrl, int argc,
					 char *argv[])
{
...
#ifdef CONFIG_ASP
static int wpa_cli_cmd_asp_seek(struct wpa_ctrl *ctrl, int argc,
				char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_SEEK", 1, argc, argv);
}


static int wpa_cli_cmd_asp_cancel_seek(struct wpa_ctrl *ctrl, int argc,
				       char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_CANCEL_SEEK", 1, argc, argv);
}


static int wpa_cli_cmd_p2p_asp_advertise(struct wpa_ctrl *ctrl, int argc,
					 char *argv[])
{
	if (argc < 3) {
		printf("Invalid ASP_ADVERTISE command: needs 3-6 arguments\n");
		return -1;
	}

	return wpa_cli_cmd(ctrl, "ASP_ADVERTISE", 3, argc, argv);
}


static int wpa_cli_cmd_p2p_asp_cancel_advertise(struct wpa_ctrl *ctrl, int argc,
						char *argv[])
{
	if (argc < 1) {
		printf("Invalid ASP_CANCEL_ADVERTISE command: needs at \
least 1 arguments\n");
		return -1;
	}

	return wpa_cli_cmd(ctrl, "ASP_CANCEL_ADVERTISE", 1, argc, argv);
}

static int wpa_cli_cmd_asp_connect_session(struct wpa_ctrl *ctrl, int argc,
					   char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_CONNECT_SESSION", 3, argc, argv);
}


static int wpa_cli_cmd_asp_remove_session(struct wpa_ctrl *ctrl, int argc,
					   char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_REMOVE_SESSION", 3, argc, argv);
}

static int wpa_cli_cmd_asp_bound_port(struct wpa_ctrl *ctrl, int argc,
				      char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_BOUND_PORT", 4, argc, argv);
}

static int wpa_cli_cmd_asp_release_port(struct wpa_ctrl *ctrl, int argc,
					char *argv[])
{
	return wpa_cli_cmd(ctrl, "ASP_RELEASE_PORT", 4, argc, argv);
}
#endif
...
static const struct wpa_cli_cmd wpa_cli_commands[] = {
	...
	{ "p2p_asp_provision", wpa_cli_cmd_asp_provision, NULL,
	  cli_cmd_flag_none,
...
#ifdef CONFIG_ASP
	{ "asp_seek", wpa_cli_cmd_asp_seek, NULL,
	  cli_cmd_flag_none,
	  "[timeout] [type=*] = seek ASP services for up-to timeout seconds" },
	{ "asp_cancel_seek", wpa_cli_cmd_asp_cancel_seek, NULL, cli_cmd_flag_none,
	  "= stop ASP seek" },
	{ "asp_advertise", wpa_cli_cmd_p2p_asp_advertise, NULL,
	  cli_cmd_flag_none,
	  "add a local service" },
	{ "asp_cancel_advertise", wpa_cli_cmd_p2p_asp_cancel_advertise, NULL,
	  cli_cmd_flag_none,
	  "cancel advertising a local service" },
	{ "asp_connect_session", wpa_cli_cmd_asp_connect_session, NULL,
	  cli_cmd_flag_none,
	  "<addr> adv_id=<adv_id> conncap=<conncap> [info=<infodata>] = establish a session with an ASP service advertiser" },
	{ "asp_remove_session", wpa_cli_cmd_asp_remove_session, NULL,
	  cli_cmd_flag_none,
	  "<addr> session_mac=<addr> session_id=<session_id> reason=<reason> = remove an ASP session" },
	{ "asp_bound_port", wpa_cli_cmd_asp_bound_port, NULL,
	  cli_cmd_flag_none,
	  "<session_mac> <session_id> <port> <prot> = notify bound port" },
	{ "asp_release_port", wpa_cli_cmd_asp_release_port, NULL,
	  cli_cmd_flag_none,
	  "<session_mac> <session_id> <port> <prot> = notify release port" },
#endif /* CONFIG_ASP */
#endif /* CONFIG_P2P */
...
static void wpa_cli_action_process(const char *msg)
{
	...
	if (str_starts(pos, WPA_EVENT_CONNECTED)) {
		...
		wpa_cli_exec(action_file, ifname, pos);
	} else if (str_starts(pos, AP_REJECTED_MAX_STA)) {
		wpa_cli_exec(action_file, ifname, pos);
	} else if (str_starts(pos, REACH_HIGH_THERMAL)) {
		wpa_cli_exec(action_file, ifname, pos);
