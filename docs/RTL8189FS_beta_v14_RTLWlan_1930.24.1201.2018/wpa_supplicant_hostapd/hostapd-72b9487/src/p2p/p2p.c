...
#ifdef CONFIG_ASP
	if (p2p->cfg->prov_disc_req_sent)
		p2p->cfg->prov_disc_req_sent(p2p->cfg->cb_ctx, success);
#endif
...
#ifdef CONFIG_ASP
	if (p2p->cfg->go_neg_req_sent)
		p2p->cfg->go_neg_req_sent(p2p->cfg->cb_ctx, success);
#endif
	if (success) {
		if (dev->flags & P2P_DEV_USER_REJECTED) {
			p2p_set_state(p2p, P2P_IDLE);
