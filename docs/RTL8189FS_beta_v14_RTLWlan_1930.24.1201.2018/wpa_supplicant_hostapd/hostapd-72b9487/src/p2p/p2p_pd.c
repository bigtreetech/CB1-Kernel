...
static void p2p_encaps_prov_disc(struct wpabuf *buf, struct wpabuf *subelems)
{
	const u8 *pos, *end;
					       ...
					       int join)
{
	...
	p2p_subelems = wpabuf_alloc(wpabuf_size(buf));
	...
	p2p_buf_add_capability(p2p_subelems, p2p->dev_capab &
			       ~P2P_DEV_CAPAB_CLIENT_DISCOVERABILITY,
			       group_capab);
	p2p_buf_add_device_info(p2p_subelems, p2p, NULL);
	if (p2p->p2ps_prov) {
		p2ps_add_pd_req_attrs(p2p, dev, p2p_subelems, config_methods);
	} else if (go) {
		p2p_buf_add_group_id(p2p_subelems, go->info.p2p_device_addr,
				     go->oper_ssid, go->oper_ssid_len);
	}

	p2p_encaps_prov_disc(buf, p2p_subelems);
...
#ifdef CONFIG_ASP
		if (p2p->cfg->p2ps_prov_disc_req_recvd)
			p2p->cfg->p2ps_prov_disc_req_recvd(p2p->cfg->cb_ctx,
							   sa, msg.device_name,
							   0, 0,
							   adv_mac,
							   session_mac,
				...
				const u8 *data, size_t len)
{
	...
	if ((status == P2P_SC_SUCCESS || status == P2P_SC_SUCCESS_DEFERRED) &&
		...
		if (p2p->cfg->p2ps_prov_complete) {
			...
			p2p->cfg->p2ps_prov_complete(
				p2p->cfg->cb_ctx, p2p->p2ps_prov, status, sa, adv_mac,
