...
static int yt8614_read_status(struct phy_device *phydev)
{
	...
	if (YT8614_PHY_MODE_CURR & YT_PHY_MODE_UTP) {
		...
		if (link) {
			if (phydev->link == 0)
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
				netdev_info(phydev->attached_dev, "%s, phy addr: %d, link up, UTP, reg 0x11 = 0x%x\n",
	...
	if (YT8614_PHY_MODE_CURR & YT_PHY_MODE_FIBER) {
		...
		if (link) {
			if (phydev->link == 0)
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
				netdev_info(phydev->attached_dev, "%s, phy addr: %d, link up, Fiber, reg 0x11 = 0x%x\n",
					__func__, phydev->addr, (unsigned int)val);
#else
				netdev_info(phydev->attached_dev, "%s, phy addr: %d, link up, Fiber, reg 0x11 = 0x%x\n",
					__func__, phydev->mdio.addr, (unsigned int)val);
...
static int yt8821_read_status(struct phy_device *phydev)
{
	...
	if (link_utp || link_fiber) {
		if (phydev->link == 0)
#if (KERNEL_VERSION(4, 5, 0) > LINUX_VERSION_CODE)
			netdev_info(phydev->attached_dev, "%s, phy addr: %d, link up, media %s, reg 0x11 = 0x%x\n",
				__func__, phydev->addr, (link_utp && link_fiber) ? "both UTP and Fiber" : (link_utp ? "UTP" : "Fiber"), (unsigned int)val);
#else
			netdev_info(phydev->attached_dev, "%s, phy addr: %d, link up, media %s, reg 0x11 = 0x%x\n",
				__func__, phydev->mdio.addr, (link_utp && link_fiber) ? "both UTP and Fiber" : (link_utp ? "UTP" : "Fiber"), (unsigned int)val);
#endif
		phydev->link = 1;
	} else {
		if (phydev->link == 1)
