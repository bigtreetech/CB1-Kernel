		     ...
		     ASN1_TLC *ctx)
{
	...
	const unsigned char cinf[][13] = {
		"cert_info", "version",    "serialNumber", "signature",
		"algorithm", "parameter",  "isure",	"vald",
		"suject",    "key",	"algor",	"algorithm",
		"parameter", "public_key", "extensions"
	};
	...
	case ASN1_ITYPE_MSTRING:
		...
		ret = asn1_check_tlen(NULL, &otag, &oclass, NULL, NULL, &p, len,
				      -1, 0, 1, ctx);
		if (!ret) {
			ASN1err(ASN1_F_ASN1_ITEM_EX_D2I,
	...
	case ASN1_ITYPE_EXTERN:
		ef = it->funcs;
		return ef->asn1_ex_d2i(pval, in, len, it, tag, aclass, opt,
				       ctx); //??????
#endif
#if COMPAT
	case ASN1_ITYPE_COMPAT:
		...
		if (opt) {
			...
			ret = asn1_check_tlen(NULL, NULL, NULL, NULL, NULL, &p,
					      len, exptag, aclass, 1, ctx);
			if (!ret) {
				ASN1err(ASN1_F_ASN1_ITEM_EX_D2I,
					ERR_R_NESTED_ASN1_ERROR);
			}
	...
	case ASN1_ITYPE_SEQUENCE:
		...
		ret = asn1_check_tlen(&len, NULL, NULL, &seq_eoc, &cst, &p, len,
				      tag, aclass, opt, ctx); //1
		if (!ret) {
			ASN1err(ASN1_F_ASN1_ITEM_EX_D2I,
				ERR_R_NESTED_ASN1_ERROR);
		} else if (ret == -1)
		...
		for (i = 0, tt = it->templates; i < it->tcount;
		     i++, tt++) //x509_CINF_seq_tt
		{
			const ASN1_TEMPLATE *seqtt;
			ASN1_VALUE **pseqval;
			seqtt   = asn1_do_adb(pval, tt, 1);
			pseqval = asn1_get_field_ptr(
				pval,
				seqtt); //获得x509_CINF_seq_tt子类型的偏移量
			//	printf("%s\n",seqtt->field_name);
			...
			for (j = 0 + k; j < 17; j++) {
				...
				if (!strcmp((const char *)cinf[j],
					    seqtt->field_name)) {
					k++;
					break;
				} else {
					asn1_check_tlen(&len, NULL, NULL,
							&seq_eoc, &cst, &p, len,
							tag, aclass, opt, ctx);
					p   = p + len;
					*in = p;
			...
			if (i == (it->tcount - 1))
				...
				isopt = (char)(seqtt->flags &
					       ASN1_TFLG_OPTIONAL); //differrn

			ret = asn1_template_ex_d2i(
				pseqval, &p,
				len, //这里开始对子template，进行转化
				seqtt, isopt, ctx);
			if (!ret) {
				//errtt = seqtt;
			} else if (ret == -1) {
			   ...
			   ASN1_TLC *ctx)
{
	...
	if (ctx && ctx->valid) {
		...
		if (ctx) {
			...
			ctx->hdrlen =
				p -
				q; //这里用了多少个字节即0x30,0x82,0x04,0x52,4个
			ctx->valid = 1;
	...
	if (exptag >= 0) {
		if ((exptag != ptag) ||
		    (expclass != pclass)) //different,ptag=0,expclass=128
		{
			if (opt)
				return -1;
			asn1_tlc_clear(ctx);
				 ...
				 int aclass, char opt, ASN1_TLC *ctx)
{
	...
	if (utype == V_ASN1_ANY) {
		...
		p   = *in;
		ret = asn1_check_tlen(NULL, &utype, &oclass, NULL, NULL, &p,
				      inlen, -1, 0, 0, ctx);
		if (!ret) {
			ASN1err(ASN1_F_ASN1_D2I_EX_PRIMITIVE,
				ERR_R_NESTED_ASN1_ERROR);
	...
	if (!asn1_ex_c2i(pval, cont, len, utype, &free_cont,
			 it)) //asn1 to interger
		goto err;
				   ...
				   ASN1_TLC *ctx)
{
	...
	if (flags & ASN1_TFLG_SK_MASK) {
		...
		ret = asn1_check_tlen(&len, NULL, NULL, &sk_eoc, NULL, &p, len,
				      sktag, skaclass, opt, ctx);
		if (!ret) {
		...
		ret = ASN1_item_ex_d2i(
			val, &p, len,
			ASN1_ITEM_ptr(tt->item), //子tempate,INTERGER#####
			-1, 0, opt, ctx);
		if (!ret) {
			ASN1err(ASN1_F_ASN1_TEMPLATE_NOEXP_D2I,
				ERR_R_NESTED_ASN1_ERROR);
				...
				ASN1_TLC *ctx)
{
	...
	if (flags & ASN1_TFLG_EXPTAG) {
		...
		ret = asn1_check_tlen(&len, NULL, NULL, &exp_eoc,
				      &cst, //interger,
		...
		ret = asn1_template_noexp_d2i(val, &p, len, tt, 0,
					      ctx); //len=3,p=0x42c5ca
		if (!ret) {
			ASN1err(ASN1_F_ASN1_TEMPLATE_EX_D2I,
				ERR_R_NESTED_ASN1_ERROR);
			return 0;
