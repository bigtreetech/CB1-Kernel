...
************************************************************************************************************
*/
static int __certif_probe_signdata(u8 *dst_buf, u32 dst_len_max, u8 *src_buf,
				   u32 src_len)
{
	u8 *tmp_buf = src_buf;
	int ret;
...
static uint __merge_extension_value(u8 **dst_buf, u8 *src_buf, uint src_buf_len)
{
	...
	*dst_buf = malloc((asn1.data_len + 1) / 2);
	...
	if (tmp_len > 512) //rsakey
	{
		...
		if (sunxi_bytes_merge(
			    *dst_buf + 512 / 2, asn1.data_len, src + 512,
			    asn1.data_len - 512 - (src - asn1.data))) {
			printf("__merge_extension_value err2: in sunxi_bytes_merge\n");

			return 0;
		}
	} else {
		if (sunxi_bytes_merge(*dst_buf, asn1.data_len, asn1.data,
				      asn1.data_len)) {
			printf("__merge_extension_value err1: in sunxi_bytes_merge\n");
...
int sunxi_certif_probe_extension(X509 *x, sunxi_certif_info_t *sunxi_certif)
{
	...
	for (i = 0; i < extension_count; i++) {
		...
		extension =
			sk_X509_EXTENSION_value(x->cert_info->extensions, i);
		if (!extension) {
		...
		if (!len) {
			...
			len = __merge_extension_value(
				&sunxi_certif->extension.value[i], buff_value,
				len);
			if (!len) {
			       ...
			       u32 len)
{
	...
	p_sign_to_calc = malloc_cache_aligned(4096); //证书中待签名内容肯定不超过4k
	//获取待签名内容
	...
	if (sign_src_len <= 0) {
		...
		goto err_sign_buf_created;
	}
	//计算待签名内容的hash
	memset(hash_of_certif, 0, sizeof(hash_of_certif));
	ret = sunxi_sha_calc(hash_of_certif, sizeof(hash_of_certif),
			     p_sign_to_calc, sign_src_len);
	if (ret) {
	...
	ret = sunxi_rsa_calc(sunxi_certif->pubkey.n + 1,
			     sunxi_certif->pubkey.n_len - 1,
			     sunxi_certif->pubkey.e, sunxi_certif->pubkey.e_len,
			     hash_of_sign, sizeof(hash_of_sign), sign_in_certif,
			     sizeof(sign_in_certif));
	if (ret) {
