...
static int aw_spinand_chip_reset(struct aw_spinand_chip *chip)
{
	...
	ret = spi0_write(txbuf, 1, SPI0_MODE_AUTOSET);
	if (ret)
		return ret;
...
{
	int ret;
	char txbuf[2] = {SPI_NAND_RDID, 0x00};
	...
	if (dummy)
		return spi0_write_then_read(txbuf, 2, id, len,
				SPI0_MODE_AUTOSET);
	else
		return spi0_write_then_read(txbuf, 1, id, len,
				SPI0_MODE_AUTOSET);
}
		...
		unsigned char cmd, unsigned char reg, unsigned char *val)
{
	...
	return spi0_write_then_read(txbuf, 2, val, 1,
			SPI0_MODE_AUTOSET);
}

static int aw_spinand_chip_write_reg(struct aw_spinand_chip *chip, unsigned char cmd,
		unsigned char reg, unsigned char val)
{
	...
	return spi0_write(txbuf, 3, SPI0_MODE_AUTOSET);
}
...
static int aw_spinand_chip_write_enable(struct aw_spinand_chip *chip)
{
	...
	return spi0_write(txbuf, 1, SPI0_MODE_AUTOSET);
}
		...
		struct aw_spinand_chip_request *req)
{
	...
	ret = spi0_write(txbuf, 4, SPI0_MODE_AUTOSET);
	if (ret)
		return ret;
		...
		struct aw_spinand_chip_request *req)
{
	...
	return spi0_write(txbuf, 4, SPI0_MODE_AUTOSET);
}
