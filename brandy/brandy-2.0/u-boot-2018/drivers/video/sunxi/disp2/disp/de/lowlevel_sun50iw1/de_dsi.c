...
static volatile	__de_dsi_dphy_dev_t	*dphy_dev[2];
...
{
	u32 dsi_start_delay =	dsi_dev[sel]->dsi_basic_ctl1.bits.video_start_delay;
...
s32 dsi_dcs_rd(u32 sel,u8	cmd,u8* para_p,u32*	num_p)
{
	...
	if (dsi_dev[sel]->dsi_cmd_ctl.bits.rx_flag)
	{
		...
		else if (dsi_dev[sel]->dsi_cmd_rx[0].bits.byte0==(u8)DSI_DT_DCS_RD_R2)
		{
			num	= 2;
			*(para_p+0)	= dsi_dev[sel]->dsi_cmd_rx[0].bits.byte1;
			*(para_p+1)	= dsi_dev[sel]->dsi_cmd_rx[0].bits.byte2;
		}
		else if (dsi_dev[sel]->dsi_cmd_rx[0].bits.byte0==(u8)DSI_DT_DCS_LONG_RD_R)
		{
			num	= dsi_dev[sel]->dsi_cmd_ctl.bits.rx_size+1-6;
			for (i=0;i<num;i++)
			{
				*(para_p+i)	= *((u8*)dsi_dev[sel]->dsi_cmd_rx+4+i);
			}
		}
		else
		{
...
static s32 dsi_basic_cfg(u32 sel,disp_panel_para *	panel)
{
	...
	else
	{
		...
		/* put start_delay to tcon. set ready sync early to dramfreq, so set start_delay 1 */
		start_delay = 1;
...
static s32 dsi_packet_cfg(u32 sel,disp_panel_para * panel)
{
	...
	{
		u32 dsi_hsa,dsi_hact,dsi_hbp,dsi_hfp,dsi_hblk,dsi_vblk,tmp;
		u32 vc = 0;
		u32 x			= panel->lcd_x;
		u32 y			= panel->lcd_y;
		u32 vt		= panel->lcd_vt;
