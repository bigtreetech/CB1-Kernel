	...
	struct disp_layer_config_data *data, u32 layer_num)
{
	...
	for (i = 0; i < layer_num; ++i, ++data) {
		if (data->config.enable
			...
			struct disp_enhance_layer_info *ehs_layer_info =
				&ehs_info.layer_info[data->config.layer_id];
	...
	ehs_info.ovl_size.width = chn_info->ovl_out_win.width;
	ehs_info.ovl_size.height = chn_info->ovl_out_win.height;
	ehs_info.bld_size.width = chn_info->scn_win.width;
	ehs_info.bld_size.height = chn_info->scn_win.height;
	/* set enhance size */
...
s32 disp_init_al(disp_bsp_init_para *para)
{
...
s32 disp_al_get_display_size(u32 disp, u32 *width, u32 *height)
{
	...
	*width = ctx->output.scn_width;
	*height = ctx->output.scn_height;
	return 0;
}
