						...
						unsigned char celut[256], hist_data* p_hist_data)
{
  ...
  if (total_size>3*total_pixel) //less than 66% pixel in valid range
	...
	if ((mean==0) || (p_hist_data->hist_mean<ce_thr) || total_size==0)	//picture too small, can't ce
	{
		for (i=0; i<256; i++)
		{
			celut[i] = i;
...
//	weight = 256;
	...
	{
		__wrn("%s, ##################error para, mean=%d, validsum=%d, validcnt=%d, pd_ymin_fix=%d, pd_black=%d, pd_white=%d, pd_ymax_fix=%d,sum=%d\n",
			__func__, mean, validsum, validcnt, pd_ymin_fix, pd_black, pd_white, pd_ymax_fix, sum);
		__wrn("hist value:\n");
		for (i=0; i<256; i+=4) {
			__wrn("0x%x 0x%x 0x%x 0x%x\n", hist[i], hist[i+1], hist[i+2], hist[i+3]);
		}
		mean=0;
		pd_ymin_fix = 16;
		pd_black = 32;
