/*
 * drivers/video/sunxi/disp2/disp/de/lowlevel_sun50iw1/de_ase.c
 *
...
int de_ase_init(unsigned int sel, unsigned int chno, uintptr_t reg_base)
{
	...
	memory = (void *)kmalloc(sizeof(__ase_reg_t), GFP_KERNEL | __GFP_ZERO);
	if (NULL == memory) {
		__wrn("malloc ase[%d][%d] memory fail! size=0x%x\n", sel, chno, (unsigned int)sizeof(__ase_reg_t));
		return -1;
	}
...
//*********************************************************************************************************************
// function       : de_ase_info2para(unsigned int auto_color, de_rect window, __ase_config_data *para)
