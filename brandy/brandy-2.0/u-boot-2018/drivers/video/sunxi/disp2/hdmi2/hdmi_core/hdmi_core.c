...
static struct disp_hdmi_mode hdmi_mode_tbl[] = {
    {DISP_TV_MOD_800x480P60,     HDMI_VIC_800x480P60, },
    {DISP_TV_MOD_1024x600P60,     HDMI_VIC_1024x600P60, },    
...
static u32 svd_user_config(u32 code, u32 refresh_rate)
{
	...
	for (i = 0; i < sizeof(hdmi_mode_tbl)/sizeof(struct disp_hdmi_mode); i++) {
        ...
        printk("==> hdmi i=%d, mode index = %d\n",i,hdmi_mode_tbl[i].mode);
	}
#endif
...
static int atoi(const char* str) {
    ...
    else if (*str<'0' || *str>'9') {  //第一个字符为其他，直接跳出
      //  return 0;
    }
...
static char hdmi_buf[50]={0};

void GetHdmiParam(void)
{
  //  printf("\n===> start setup HDMI param\n\n");
    
    run_command("mmcinfo", 0);
    run_command("mmc part", 0);
    run_command("fatload mmc 0:1 40000000 system.cfg", 0);
   // run_command("md.b 40000000 30", 0);   
    
    memcpy(hdmi_buf,(const char *)(0x40000000),60);
    
   // printf("==> hdmi mem str = %s\n",hdmi_buf); //hdmi_width:800
    
    char *p = NULL;
    
    p = strstr(hdmi_buf,"hdmi_width");
    
    if( p  !=NULL )
    {
      // printf("==> hdmi string found,%s\n",p);
       gd->hdmi_width = atoi(p+11);
    }
    
    p = strstr(hdmi_buf,"hdmi_height"); //hdmi_height:480
    
    if( p  !=NULL )
    {
      // printf("==> hdmi_height string found,%s\n",p);
       gd->hdmi_height = atoi(p+12);
    }  
    
    p = strstr(hdmi_buf,"hdmi_mode"); 
    
    if( p  !=NULL )
    {
       //printf("==> hdmi_mode string found,%s\n",p);
       gd->hdmi_mode = atoi(p+10);
    }      
 
    //printf("==> hdmi_width=%d,hdmi_height=%d\n",hdmi_width,hdmi_height);

    printf("uboot get hdmi=%dx%d mode =%d\n",gd->hdmi_width,gd->hdmi_height,gd->hdmi_mode);
    
    if(gd->hdmi_width <600 || gd->hdmi_width>10000)
    {
       gd->hdmi_width = 800; 
    }
    if(gd->hdmi_height <400 || gd->hdmi_height>10000)
    {
       gd->hdmi_height = 480; 
    }
    
    if(gd->hdmi_mode ==0 || gd->hdmi_mode > 256) gd->hdmi_mode = 69;
    
    int find_mode_flag=0;
    
    int i;
    
	for (i = 0; i < sizeof(hdmi_mode_tbl)/sizeof(struct disp_hdmi_mode); i++) {
			if (hdmi_mode_tbl[i].mode == gd->hdmi_mode) {
                    find_mode_flag = 1;
                   // printf("==> hdmi_mode_tbl[i] found mode =%d\n",i);
					break;
			}
	}  
	...
	if(!find_mode_flag)
    {
       printf("==> No found mode,use default\n");
       gd->hdmi_width = 800;  
       gd->hdmi_height = 480;
       gd->hdmi_mode = 69;
    }
...
s32 set_static_config(struct disp_device_config *config)
{
    ...
    config->mode = gd->hdmi_mode;

	hdmi_reconfig_format_by_blacklist(config);
	pVideo->update = hdmi_check_updated(core, config);
#if 0
	printf("==>uboot [HDMI receive params]: tv mode: 0x%x format:0x%x data bits:0x%x eotf:0x%x cs:0x%x "
...
s32 get_static_config(struct disp_device_config *config)
{
	...
	for (i = 0; i < sizeof(hdmi_mode_tbl)/sizeof(struct disp_hdmi_mode); i++) {
			if (hdmi_mode_tbl[i].hdmi_mode == core->mode.pVideo.mDtd.mCode) {
                    ...
                    printf("==> hdmi_mode_tbl[i] i=%d\n",i);
					break;
			}
	}
	config->mode = gd->hdmi_mode;//tv_mode;
