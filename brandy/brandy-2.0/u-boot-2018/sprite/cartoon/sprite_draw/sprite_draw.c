/*
 * sprite/cartoon/sprite_draw/sprite_draw.c
 *
...
int sprite_cartoon_ui_draw_hollowrectangle(int x1, int y1, int x2, int y2)
{
	...
	flush_cache(DO_ALIGN(cache_start - CONFIG_SYS_CACHELINE_SIZE,
			     CONFIG_SYS_CACHELINE_SIZE),
		    DO_ALIGN((sprite_source.screen_width *
				  sprite_source.screen_height * 4 +
			      CONFIG_SYS_CACHELINE_SIZE),
			     CONFIG_SYS_CACHELINE_SIZE));
...
int sprite_cartoon_ui_draw_line(int x1, int y1, int x2, int y2)
{
	...
	if (end_x == start_x) {
		...
		b = (start_x * end_y - end_x * start_y) * 1.0 /
		    (start_x - end_x);
		...
		if (ABS(end_x - start_x) > ABS(end_y - start_y)) {
			...
			base +=
			    (sprite_source.screen_width * start_y + start_x) *
			    4;
			*((int *)base) = sprite_source.color;
...
int sprite_cartoon_ui_draw_lineto(int px, int py)
{
	...
	end_x = px;
	end_y = py;
	sprite_cartoon_ui_draw_line(sprite_source.this_x, sprite_source.this_y,
				    end_x, end_y);
...
int sprite_cartoon_ui_draw_point(int x, int y)
{
	...
	base = sprite_source.screen_buf +
	       (sprite_source.screen_width * scr_y + scr_x) * 4;
	*((int *)base) = sprite_source.color;
