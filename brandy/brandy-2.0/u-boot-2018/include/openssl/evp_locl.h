...
#define BLOCK_CIPHER_func_ecb(cname, cprefix, kstruct, ksched)                 \
	...
	{                                                                      \
		BLOCK_CIPHER_ecb_loop() cprefix##_ecb_encrypt(                 \
			in + i, out + i,                                       \
			&((kstruct *)ctx->cipher_data)->ksched, ctx->encrypt); \
		return 1;                                                      \
	}

#define BLOCK_CIPHER_func_ofb(cname, cprefix, cbits, kstruct, ksched)          \
	static int cname##_ofb_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, \
				      const unsigned char *in,                 \
				      unsigned int inl)                        \
	{                                                                      \
		cprefix##_ofb##cbits##_encrypt(                                \
			in, out, (long)inl,                                    \
			&((kstruct *)ctx->cipher_data)->ksched, ctx->iv,       \
			&ctx->num);                                            \
		return 1;                                                      \
	}

#define BLOCK_CIPHER_func_cbc(cname, cprefix, kstruct, ksched)                 \
	static int cname##_cbc_cipher(EVP_CIPHER_CTX *ctx, unsigned char *out, \
				      const unsigned char *in,                 \
				      unsigned int inl)                        \
	{                                                                      \
	...
	BLOCK_CIPHER_func_cbc(cname, cprefix, kstruct, ksched)                 \
		BLOCK_CIPHER_func_cfb(cname, cprefix, cbits, kstruct, ksched)  \
			BLOCK_CIPHER_func_ecb(cname, cprefix, kstruct, ksched) \
				BLOCK_CIPHER_func_ofb(cname, cprefix, cbits,   \
						      kstruct, ksched)
...
#define BLOCK_CIPHER_def_cbc(cname, kstruct, nid, block_size, key_len, iv_len, \
			     ...
			     ctrl)                                             \
	BLOCK_CIPHER_def1(cname, cbc, cbc, CBC, kstruct, nid, block_size,      \
			  key_len, iv_len, flags, init_key, cleanup, set_asn1, \
			  get_asn1, ctrl)
...
#define BLOCK_CIPHER_def_ecb(cname, kstruct, nid, block_size, key_len, flags,  \
	...
	BLOCK_CIPHER_def1(cname, ecb, ecb, ECB, kstruct, nid, block_size,      \
			  key_len, 0, flags, init_key, cleanup, set_asn1,      \
			  get_asn1, ctrl)
...
#define BLOCK_CIPHER_defs(cname, kstruct, nid, block_size, key_len, iv_len,    \
	...
	BLOCK_CIPHER_def_cbc(cname, kstruct, nid, block_size, key_len, iv_len, \
			     flags, init_key, cleanup, set_asn1, get_asn1,     \
			     ctrl)                                             \
		BLOCK_CIPHER_def_cfb(cname, kstruct, nid, key_len, iv_len,     \
				     cbits, flags, init_key, cleanup,          \
				     set_asn1, get_asn1, ctrl)                 \
			BLOCK_CIPHER_def_ofb(cname, kstruct, nid, key_len,     \
					     iv_len, cbits, flags, init_key,   \
					     cleanup, set_asn1, get_asn1,      \
					     ctrl)                             \
				BLOCK_CIPHER_def_ecb(cname, kstruct, nid,      \
						     block_size, key_len,      \
...
#define IMPLEMENT_BLOCK_CIPHER(cname, ksched, cprefix, kstruct, nid,           \
	...
	BLOCK_CIPHER_all_funcs(cname, cprefix, cbits, kstruct, ksched)         \
		BLOCK_CIPHER_defs(cname, kstruct, nid, block_size, key_len,    \
...
#define IMPLEMENT_CFBR(cipher, cprefix, kstruct, ksched, keysize, cbits,       \
		       iv_len, fl)                                             \
	BLOCK_CIPHER_func_cfb(cipher##_##keysize, cprefix, cbits, kstruct,     \
			      ksched)                                          \
		BLOCK_CIPHER_def_cfb(cipher##_##keysize, kstruct,              \
				     NID_##cipher##_##keysize, keysize / 8,    \
				     iv_len, cbits,                            \
				     (fl) | EVP_CIPH_FLAG_DEFAULT_ASN1,        \
				     cipher##_init_key, NULL, NULL, NULL,      \
				     NULL)
