...
int main(int argc, char *argv[])
{
    ...
    ret=sprintf(buf,"%d",1);
    if(0!=set_sysfs_input_attr(class_path,"device/magnetometer/enable_device",buf,ret)){
    ...
    while(1){
        ...
        if (event.type == EV_ABS) {
            ...
            switch (event.code) {
                case ABS_X:
                    x =event.value;
                    //db_msg("ecompass senser data is: x=%d\n",x);
                    break;
                case ABS_Y:
                    y =event.value;
                    //db_msg("ecompass senser date is: y=%d\n",y);
                    break;
                case ABS_Z:
                    z =event.value;
                    //db_msg("ecompass senser date is:z=%d\n",z);
                    break;
            }
            sprintf(buf,"(%d,%d,%d)",x,y,z);
           // db_msg("ecompass senser data is:x=%d,y=%d,z=%d\n",x,y,z);
