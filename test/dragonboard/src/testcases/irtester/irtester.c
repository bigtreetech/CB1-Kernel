...
static int match_ir(char *dev_name)
{
    return strncmp(&dev_name[strlen(dev_name) - 4], "-ir", 3);
}
...
int main(int argc, char *argv[])
{
    ...
    strncpy(dev_path, "/sys/class/input/event3/device/name", 64);
    for (i = 0; i < 5; i++) {
        dev_path[22] = '0' + i;
    ...
    strncpy(evt_path, "/dev/input/event3", 32);
    evt_path[16] = '0' + i;
