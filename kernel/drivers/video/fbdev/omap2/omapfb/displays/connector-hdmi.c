...
static int hdmic_read_edid(struct omap_dss_device *dssdev, u8 *edid, int len)
{
    int r_edid;
    struct panel_drv_data *ddata = to_panel_data(dssdev);
    struct omap_dss_device *in = ddata->in;

    r_edid = in->ops.hdmi->read_edid(in, edid, len);
    printk("---> read_edid: %d \n", edit);
...
static int hdmic_probe(struct platform_device *pdev)
{
    ...
    if (gpio_is_valid(ddata->hpd_gpio))
    {
        r = devm_gpio_request_one(&pdev->dev, ddata->hpd_gpio, GPIOF_DIR_IN, "hdmi_hpd");
        if (r)
            goto err_reg;
    }
