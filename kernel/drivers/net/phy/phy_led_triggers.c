...
static struct phy_led_trigger *phy_speed_to_led_trigger(struct phy_device *phy, unsigned int speed)
{
    unsigned int i;
...
void phy_led_trigger_change_speed(struct phy_device *phy)
{
    ...
    {
        if (!phy->last_triggered)
            led_trigger_event(&phy->led_link_trigger->trigger, LED_FULL);
        else
            led_trigger_event(&phy->last_triggered->trigger, LED_OFF);
...
static void phy_led_trigger_format_name(struct phy_device *phy, char *buf, size_t size, const char *suffix)
{
...
int phy_led_triggers_register(struct phy_device *phy)
{
    ...
    phy->phy_num_led_triggers = phy_supported_speeds(phy, speeds, ARRAY_SIZE(speeds));
    if (!phy->phy_num_led_triggers)
        return 0;

    phy->led_link_trigger = devm_kzalloc(&phy->mdio.dev, sizeof(*phy->led_link_trigger), GFP_KERNEL);
    ...
    phy->phy_led_triggers = devm_kcalloc(&phy->mdio.dev, phy->phy_num_led_triggers, sizeof(struct phy_led_trigger), GFP_KERNEL);
    ...
    for (i = 0; i < phy->phy_num_led_triggers; i++)
    {
        err = phy_led_trigger_register(phy, &phy->phy_led_triggers[i], speeds[i], phy_speed_to_str(speeds[i]));
        if (err)
            goto out_unreg;
    }
