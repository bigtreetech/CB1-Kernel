...
__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_basic_features)
__ro_after_init;
EXPORT_SYMBOL_GPL(phy_basic_features);

__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_basic_t1_features)
__ro_after_init;
EXPORT_SYMBOL_GPL(phy_basic_t1_features);

__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_gbit_features)
__ro_after_init;
...
__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_10gbit_features)
__ro_after_init;
EXPORT_SYMBOL_GPL(phy_10gbit_features);

__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_10gbit_fec_features)
__ro_after_init;
...
__ETHTOOL_DECLARE_LINK_MODE_MASK(phy_10gbit_full_features)
__ro_after_init;
                           ...
                           struct phy_c45_device_ids *c45_ids)
{
    ...
    for (i = 1; i < MDIO_MMD_NUM && (devs_in_pkg == 0 ||
                                     (devs_in_pkg & 0x1fffffff) == 0x1fffffff);
         i++)
    {
                          ...
                          int addr, size_t priv_size)
{
    ...
    ptr = devres_alloc(devm_phy_package_leave, sizeof(*ptr), GFP_KERNEL);
    if (!ptr)
        return -ENOMEM;
...
static int genphy_config_advert(struct phy_device *phydev)
{
    ...
    err = phy_modify_changed(phydev, MII_CTRL1000, ADVERTISE_1000FULL | ADVERTISE_1000HALF, adv);
    if (err < 0)
        return err;
    if (err > 0)
        changed = 1;
...
static int genphy_c37_config_advert(struct phy_device *phydev)
{
    ...
    if (linkmode_test_bit(ETHTOOL_LINK_MODE_1000baseX_Full_BIT, phydev->advertising))
...
int genphy_config_eee_advert(struct phy_device *phydev)
{
    ...
    err = phy_modify_mmd_changed(phydev, MDIO_MMD_AN, MDIO_AN_EEE_ADV, phydev->eee_broken_modes, 0);
...
int genphy_setup_forced(struct phy_device *phydev)
{
    ...
    return phy_modify(phydev, MII_BMCR, ~(BMCR_LOOPBACK | BMCR_ISOLATE | BMCR_PDOWN), ctl);
}
...
int genphy_c37_config_aneg(struct phy_device *phydev)
{
    ...
    err = phy_modify(phydev, MII_BMCR, BMCR_SPEED1000 | BMCR_SPEED100, BMCR_SPEED1000);
    if (err)
        return err;
...
int genphy_c37_read_status(struct phy_device *phydev)
{
    ...
    if (phydev->autoneg == AUTONEG_ENABLE && phydev->autoneg_complete)
    {
        ...
        linkmode_mod_bit(ETHTOOL_LINK_MODE_Autoneg_BIT, phydev->lp_advertising, lpa & LPA_LPACK);
        linkmode_mod_bit(ETHTOOL_LINK_MODE_1000baseX_Full_BIT, phydev->lp_advertising, lpa & LPA_1000XFULL);
        linkmode_mod_bit(ETHTOOL_LINK_MODE_Pause_BIT, phydev->lp_advertising, lpa & LPA_1000XPAUSE);
        linkmode_mod_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, phydev->lp_advertising, lpa & LPA_1000XPAUSE_ASYM);
...
int genphy_read_abilities(struct phy_device *phydev)
{
    ...
    linkmode_set_bit_array(phy_basic_ports_array, ARRAY_SIZE(phy_basic_ports_array), phydev->supported);
...
int genphy_loopback(struct phy_device *phydev, bool enable)
{
    if (enable)
    {
        ...
        ret = phy_read_poll_timeout(phydev, MII_BMSR, val, val & BMSR_LSTATUS, 5000, 500000, true);
        if (ret)
            return ret;
    }
    else
...
{
    linkmode_mod_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, dst, linkmode_test_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, src));
    linkmode_mod_bit(ETHTOOL_LINK_MODE_Pause_BIT, dst, linkmode_test_bit(ETHTOOL_LINK_MODE_Pause_BIT, src));
}
...
{
    if (!linkmode_test_bit(ETHTOOL_LINK_MODE_Pause_BIT, phydev->supported) && pp->rx_pause)
        return false;

    if (!linkmode_test_bit(ETHTOOL_LINK_MODE_Asym_Pause_BIT, phydev->supported) && pp->rx_pause != pp->tx_pause)
        return false;
...
void phy_get_pause(struct phy_device *phydev, bool *tx_pause, bool *rx_pause)
{
    ...
    return linkmode_resolve_pause(phydev->advertising, phydev->lp_advertising, tx_pause, rx_pause);
}
...
static int phy_probe(struct device *dev)
{
    ...
    if (linkmode_test_bit(ETHTOOL_LINK_MODE_1000baseT_Full_BIT, phydev->supported))
        phydev->is_gigabit_capable = 1;
