...
int ephy_config_init(struct phy_device *phydev)
{
    ...
    if (ephy_type == EPHY_AC300) {
        int ret;
        u16 ephy_cali = 0;
        ephy_cali = (u16)sun50i_ephy_calibrate_value();
        if (ret) {
    ...
    if (ephy_type == EPHY_AC200) {
        value = acx00_reg_read(ac200_ephy.ac200, AC200_EPHY_CTL);
        if (phydev->interface == PHY_INTERFACE_MODE_RMII)
            value |= (1 << 11);
        else
            value &= (~(1 << 11));
        acx00_reg_write(ac200_ephy.ac200, AC200_EPHY_CTL, value | (1 << 11));
    } else
#endif
    if (ephy_type == EPHY_AC300) {
...
static void ac200_ephy_enable(struct ephy_res *priv)
{
    ...
    value = acx00_reg_read(priv->ac200, AC200_SYS_EPHY_CTL0);
    value |= 0x03;
    acx00_reg_write(priv->ac200, AC200_SYS_EPHY_CTL0, value);
    value = acx00_reg_read(priv->ac200, AC200_SYS_EPHY_CTL1);

    value |= 0x0f;

    acx00_reg_write(priv->ac200, AC200_SYS_EPHY_CTL1, value);
    acx00_reg_write(priv->ac200, AC200_EPHY_CTL, 0x06);

    /*for ephy */
    value = acx00_reg_read(priv->ac200, AC200_EPHY_CTL);
    value &= ~(0xf << 12);
    ...
    value |= (0x0F & (0x03 + ephy_cali)) << 12;
#else
    value |= (0x0F & (0x03 + acx00_reg_read(priv->ac200, AC200_SID))) << 12;
#endif

    acx00_reg_write(priv->ac200, AC200_EPHY_CTL, value);
