                              ...
                              int card_drv_type, int *drv_type)
{
    ...
    *drv_type = 0;
     ...
     */
    return host->ops->select_drive_strength(card, max_dtr, host_drv_type, card_drv_type, drv_type);
}
                                            ...
                                            unsigned int arg)
{
    ...
    unsigned int max_busy_timeout = host->max_busy_timeout ? host->max_busy_timeout : MMC_ERASE_TIMEOUT_MS;
...
unsigned int mmc_calc_max_discard(struct mmc_card *card)
{
    ...
    pr_debug("%s: calculated max. discard sectors %u for timeout %u ms\n",
             mmc_hostname(host), max_discard, host->max_busy_timeout ? host->max_busy_timeout : MMC_ERASE_TIMEOUT_MS);
    return max_discard;
}
