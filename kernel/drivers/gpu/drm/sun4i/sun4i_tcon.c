...
static struct drm_connector *sun4i_tcon_get_connector(const struct drm_encoder *encoder)
{
    ...
    drm_for_each_connector_iter(connector, &iter) if (connector->encoder == encoder)
    {
                                 ...
                                 const struct drm_display_mode *mode)
{
    ...
    regmap_write(tcon->regs, SUN4I_TCON1_BASIC4_REG,
                 SUN4I_TCON1_BASIC4_V_TOTAL(vtotal) | SUN4I_TCON1_BASIC4_V_BACKPORCH(bp));
...
static void sun4i_tcon_finish_page_flip(struct drm_device *dev, struct sun4i_crtc *scrtc)
{
    unsigned long flags;
...
static int sun4i_tcon_init_clocks(struct device *dev, struct sun4i_tcon *tcon)
{
...
static int sun4i_tcon_init_irq(struct device *dev, struct sun4i_tcon *tcon)
{
...
static int sun4i_tcon_get_index(struct sun4i_drv *drv)
{
     ...
     */
    list_for_each(pos, &drv->tcon_list)++ size;
 ...
 * works.
 */
static struct sunxi_engine *sun4i_tcon_find_engine(struct sun4i_drv *drv, struct device_node *node)
{
...
static int sun4i_tcon_bind(struct device *dev, struct device *master, void *data)
{
