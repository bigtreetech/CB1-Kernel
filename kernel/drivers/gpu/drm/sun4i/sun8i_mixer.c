...
static void sun8i_mixer_de33_init(struct sun8i_mixer *mixer)
{
    ...
    /* Enable the mixer */
    regmap_write(mixer->top_regs, SUN50I_MIXER_GLOBAL_CTL, SUN8I_MIXER_GLOBAL_CTL_RT_EN);
...
static int sun8i_mixer_bind(struct device *dev, struct device *master, void *data)
{
    ...
    mixer->engine.regs = devm_regmap_init_mmio(dev, regs, &sun8i_mixer_regmap_config);
...
static void sun8i_mixer_unbind(struct device *dev, struct device *master, void *data)
{
