...
void sun8i_ui_scaler_enable(struct sun8i_mixer *mixer, int layer, bool enable)
{
    ...
    if (enable)
        val = SUN8I_SCALER_GSU_CTRL_EN | SUN8I_SCALER_GSU_CTRL_COEFF_RDY;
    else
        val = 0;
                           ...
                           u32 hscale, u32 vscale, u32 hphase, u32 vphase)
{
    ...
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_OUTSIZE(base), outsize);
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_INSIZE(base), insize);
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_HSTEP(base), hscale);
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_VSTEP(base), vscale);
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_HPHASE(base), hphase);
    regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_VPHASE(base), vphase);
    offset = sun8i_ui_scaler_coef_index(hscale) * SUN8I_UI_SCALER_COEFF_COUNT;

    for (i = 0; i < SUN8I_UI_SCALER_COEFF_COUNT; i++)
        regmap_write(mixer->engine.regs, SUN8I_SCALER_GSU_HCOEFF(base, i), lan2coefftab16[offset + i]);
}
