...
mv64xxx_i2c_fsm(struct mv64xxx_i2c_data *drv_data, u32 status)
{
    ...
    case MV64XXX_I2C_STATUS_MAST_WR_ACK:        /* 0x28 */
        if ((drv_data->bytes_left == 0) || (drv_data->aborting && (drv_data->byte_posn != 0)))
        {
...
static int mv64xxx_i2c_init_recovery_info(struct mv64xxx_i2c_data *drv_data, struct device *dev)
{
...
mv64xxx_i2c_probe(struct platform_device *pd)
{
    ...
    drv_data = devm_kzalloc(&pd->dev, sizeof(struct mv64xxx_i2c_data), GFP_KERNEL);
    if (!drv_data)
        return -ENOMEM;
    ...
    if (rc)
    {
        dev_err(&drv_data->adapter.dev, "mv64xxx: Can't register intr handler irq%d: %d\n", drv_data->irq, rc);
        goto exit_disable_pm;
    }
    else if ((rc = i2c_add_numbered_adapter(&drv_data->adapter)) != 0)
    {
        dev_err(&drv_data->adapter.dev, "mv64xxx: Can't add i2c adapter, rc: %d\n", -rc);
        goto exit_free_irq;
    }
...
static const struct dev_pm_ops mv64xxx_i2c_pm_ops = {
    SET_RUNTIME_PM_OPS(mv64xxx_i2c_runtime_suspend, mv64xxx_i2c_runtime_resume, NULL)
