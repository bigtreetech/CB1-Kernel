...
#define USB_DEVICE_HID_CLASS(vend, prod)                                       \
    .match_flags = USB_DEVICE_ID_MATCH_INT_CLASS | USB_DEVICE_ID_MATCH_DEVICE, \
    .idVendor = (vend),                                                        \
    .idProduct = (prod),                                                       \
...
#ifdef MULTI_PACKET
static void usbtouch_process_multi(struct usbtouch_usb *usbtouch, unsigned char *pkt, int len)
{
    unsigned char *buffer;
...
static int usbtouch_reset_resume(struct usb_interface *intf)
{
    ...
    if (usbtouch->type->init)
    {
        ...
        if (err)
        {
            dev_dbg(&intf->dev, "%s - type->init() failed, err: %d\n", __func__, err);
            return err;
        }
    }
...
{
    usb_free_coherent(udev, usbtouch->data_size, usbtouch->data, usbtouch->data_dma);
...
static int usbtouch_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    if (type->get_pkt_len)
    {
         ...
         * nothing else.
         */
        usbtouch->data_size = min(usbtouch->data_size, usb_endpoint_maxp(endpoint));
    }
