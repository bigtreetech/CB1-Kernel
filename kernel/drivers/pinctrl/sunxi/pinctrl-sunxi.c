...
static const char *sunxi_pctrl_get_group_name(struct pinctrl_dev *pctldev, unsigned group)
{
    struct sunxi_pinctrl *pctl = pinctrl_dev_get_drvdata(pctldev);
                                      ...
                                      unsigned *num_maps)
{
    ...
    *map = kmalloc_array(nmaps, sizeof(struct pinctrl_map), GFP_KERNEL);
    ...
    {
        struct sunxi_pinctrl_group *grp = sunxi_pinctrl_find_group_by_name(pctl, group);
        ...
        if (!sunxi_pinctrl_desc_find_function_by_name(pctl, grp->name, function))
        {
            dev_err(pctl->dev, "unsupported function %s on pin %s", function, group);
            continue;
        }
...
static int sunxi_pinctrl_irq_request_resources(struct irq_data *d)
{
    ...
    func = sunxi_pinctrl_desc_find_function_by_pin(pctl, pctl->irq_array[d->hwirq], "irq");
    if (!func)
        return -EINVAL;

    ret = gpiochip_lock_as_irq(pctl->chip, pctl->irq_array[d->hwirq] - pctl->desc->pin_base);
    if (ret)
    {
...
static void sunxi_pinctrl_irq_release_resources(struct irq_data *d)
{
    ...
    gpiochip_unlock_as_irq(pctl->chip, pctl->irq_array[d->hwirq] - pctl->desc->pin_base);
}
...
static int sunxi_pinctrl_irq_set_type(struct irq_data *d, unsigned int type)
{
    ...
    if (type & IRQ_TYPE_LEVEL_MASK)
        irq_set_chip_handler_name_locked(d, &sunxi_pinctrl_level_irq_chip, handle_fasteoi_irq, NULL);
    else
        irq_set_chip_handler_name_locked(d, &sunxi_pinctrl_edge_irq_chip, handle_edge_irq, NULL);
...
static void sunxi_pinctrl_irq_handler(struct irq_desc *desc)
{
    ...
    if (val)
    {
        ...
        for_each_set_bit(irqoffset, &val, IRQ_PER_BANK)
            generic_handle_domain_irq(pctl->domain, bank * IRQ_PER_BANK + irqoffset);
    }

    chained_irq_exit(chip, desc);
}

static int sunxi_pinctrl_add_function(struct sunxi_pinctrl *pctl, const char *name)
{
    struct sunxi_pinctrl_function *func = pctl->functions;
...
static int sunxi_pinctrl_build_state(struct platform_device *pdev)
{
     ...
     */
    pctl->groups = devm_kcalloc(&pdev->dev, pctl->desc->npins, sizeof(*pctl->groups), GFP_KERNEL);
    if (!pctl->groups)
        return -ENOMEM;
     ...
     */
    pctl->functions = kcalloc(4 * pctl->ngroups + 4, sizeof(*pctl->functions), GFP_KERNEL);
    if (!pctl->functions)
        return -ENOMEM;
    ...
    ptr = krealloc(pctl->functions, pctl->nfunctions * sizeof(*pctl->functions), GFP_KERNEL);
    if (!ptr)
    {
    ...
    for (i = 0; i < pctl->desc->npins; i++)
    {
        ...
        for (func = pin->functions; func->name; func++)
        {
            ...
            func_item = sunxi_pinctrl_find_function_by_name(pctl, func->name);
            if (!func_item)
            {
                kfree(pctl->functions);
                return -EINVAL;
            ...
            {
                func_item->groups =
                    devm_kcalloc(&pdev->dev, func_item->ngroups, sizeof(*func_item->groups), GFP_KERNEL);
                if (!func_item->groups)
                {
                    kfree(pctl->functions);
                    return -ENOMEM;
...
static int sunxi_pinctrl_setup_debounce(struct sunxi_pinctrl *pctl, struct device_node *node)
{
    ...
    for (i = 0; i < pctl->desc->irq_banks; i++)
    {
        ...
        ret = of_property_read_u32_index(node, "input-debounce", i, &debounce);
        if (ret)
            return ret;
        ...
        writel(src | div << 4, pctl->membase + sunxi_irq_debounce_reg_from_bank(pctl->desc, i));
    }
                                    ...
                                    unsigned long variant)
{
    ...
    pctl->irq = devm_kcalloc(&pdev->dev, pctl->desc->irq_banks, sizeof(*pctl->irq), GFP_KERNEL);
    if (!pctl->irq)
    {
        ret = -ENOMEM;
        goto clk_error;
    ...
    for (i = 0; i < (pctl->desc->irq_banks * IRQ_PER_BANK); i++)
    {
        ...
        irq_set_lockdep_class(irqno, &sunxi_pinctrl_irq_lock_class, &sunxi_pinctrl_irq_request_class);
        irq_set_chip_and_handler(irqno, &sunxi_pinctrl_edge_irq_chip, handle_edge_irq);
    ...
    for (i = 0; i < pctl->desc->irq_banks; i++)
    {
        ...
        writel(0, pctl->membase + sunxi_irq_ctrl_reg_from_bank(pctl->desc, i));
        writel(0xffffffff, pctl->membase + sunxi_irq_status_reg_from_bank(pctl->desc, i));

        irq_set_chained_handler_and_data(pctl->irq[i], sunxi_pinctrl_irq_handler, pctl);
    }
