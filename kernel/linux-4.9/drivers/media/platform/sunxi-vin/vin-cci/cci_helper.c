...
int cci_read_a16_d32(struct v4l2_subdev *sd, unsigned short addr,
		     unsigned int *value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);

	return cci_rd_16_32(cci_drv->cci_id, addr, value, cci_drv->cci_saddr);
#else
...
EXPORT_SYMBOL_GPL(cci_read_a16_d32);
...
int cci_write_a16_d32(struct v4l2_subdev *sd, unsigned short addr,
		unsigned int value)
{
#ifdef USE_SPECIFIC_CCI
	struct cci_driver *cci_drv = v4l2_get_subdevdata(sd);

	return cci_wr_16_32(cci_drv->cci_id, addr, value, cci_drv->cci_saddr);
#else
...
EXPORT_SYMBOL_GPL(cci_write_a16_d32);
