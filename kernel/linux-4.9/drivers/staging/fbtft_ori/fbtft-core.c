					...
					struct fbtft_platform_data *pdata)
{
	...
	if (!vmem)
    {
        printk("==> tftfb malloc error: vmem\n");
		goto alloc_fail;
    }
	...
	if (!fbops)
    {
        printk("==> tftfb malloc error: fbops\n");
		goto alloc_fail;
    }
	...
	if (!fbdefio)
    {
        printk("==> tftfb malloc error: fbdefio\n");
		goto alloc_fail;
    }
	...
	if (!info)
    {
         printk("==> tftfb malloc error: framebuffer_alloc\n");
		goto alloc_fail;
    }
	...
	if (par->gamma.curves && gamma) {
        ...
        {
            printk("==> tftfb malloc error: gamma\n");
			goto alloc_fail;
        }
	}
	...
	if (txbuflen > 0) {
		...
		if (!txbuf)
        {
            printk("==> tftfb malloc error: txbuf\n");
			//goto alloc_fail;
        }
		par->txbuf.buf = txbuf;
			...
			struct spi_device *sdev, struct platform_device *pdev)
{
	...
	if (!pdata) {
		...
		if (IS_ERR(pdata)){
            printk("===> fbtft_probe_common: pdata error\n");
	...
	if (!info)
    {
        printk("===> fbtft_probe_common: alloc error\n");
		return -ENOMEM;
    }
	...
	if (ret < 0){
        printk("===> fbtft_probe_common: egister_framebuffer error\n");
		goto out_release;
    }
