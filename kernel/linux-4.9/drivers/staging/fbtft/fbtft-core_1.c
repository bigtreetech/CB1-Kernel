...
int fbtft_write_buf_dc(struct fbtft_par *par, void *buf, size_t len, int dc)
{
	...
	//if (par->gpio.dc)
        ...
        __gpio_set_value(GPIO_LCD_DC,dc);
				  ...
				  struct gpio_desc **gpiop)
{
	...
	*gpiop = devm_gpiod_get_index_optional(dev, name, index,GPIOD_OUT_HIGH);
...
static int fbtft_request_gpios(struct fbtft_par *par)
{
 ...
 #if 0 
    int gpiod_ts_irq=0,gpiod_ts_mosi=0,gpiod_ts_miso=0,gpiod_ts_clk=0;

 	gpiod_ts_irq = of_get_named_gpio_flags(np, "ts_irq",0, (enum of_gpio_flags *)&config);
 
	if (!gpio_is_valid(gpiod_ts_irq)) 
    {
		printk("---get gpio ts_irq failed\n");
	} 
	else 
    {
		printk("--- gpiod_ts_irq: gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",config.gpio,config.mul_sel,config.pull,config.drv_level,config.data);
    }
    

 	gpiod_ts_mosi = of_get_named_gpio_flags(np, "ts_mosi",0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(gpiod_ts_mosi)) 
    {
		printk("---get gpio gpiod_ts_mosi failed\n");
	} 
	else 
    {
		printk("--- gpiod_ts_mosi: gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",config.gpio,config.mul_sel,config.pull,config.drv_level,config.data);
    }
  
 	gpiod_ts_miso = of_get_named_gpio_flags(np, "ts_miso",0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(gpiod_ts_miso)) 
    {
		printk("---get gpio gpiod_ts_miso failed\n");
	} 
	else 
    {
		printk("--- gpiod_ts_miso: gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",config.gpio,config.mul_sel,config.pull,config.drv_level,config.data);
    }    

  	gpiod_ts_clk = of_get_named_gpio_flags(np, "ts_clk",0, (enum of_gpio_flags *)&config);
	if (!gpio_is_valid(gpiod_ts_clk)) 
    {
		printk("---get gpio gpiod_ts_clk failed\n");
	} 
	else 
    {
		printk("--- gpiod_ts_clk: gpio=%d  mul-sel=%d  pull=%d  drv_level=%d  data=%d\n",config.gpio,config.mul_sel,config.pull,config.drv_level,config.data);
    } 
     
 	ret = devm_gpio_request(par->info->device, gpiod_ts_irq,"ts_irq");
	if (ret < 0) {
		dev_err(par->info->device, "----- can't request ts_irq gpio %d\n",gpiod_ts_irq);
	}   

 	ret = devm_gpio_request(par->info->device, gpiod_ts_mosi,"ts_mosi");
	if (ret < 0) {
		dev_err(par->info->device, "----- can't request ts_mosi gpio %d\n",gpiod_ts_mosi);
	} 
	
 	ret = devm_gpio_request(par->info->device, gpiod_ts_miso,"ts_miso");
	if (ret < 0) {
		dev_err(par->info->device, "----- can't request ts_miso gpio %d\n",gpiod_ts_miso);
	} 
	
 	ret = devm_gpio_request(par->info->device, gpiod_ts_clk,"ts_clk");
	if (ret < 0) {
		dev_err(par->info->device, "----- can't request ts_clk gpio %d\n",gpiod_ts_clk);
	} 
	
	printk("===> get ts gpio num: irq = %d , mosi = %d , miso = %d , clk = %d \n",gpiod_ts_irq,gpiod_ts_mosi,gpiod_ts_miso,gpiod_ts_clk);
	
	gpio_direction_input(gpiod_ts_irq);
	gpio_direction_output(gpiod_ts_mosi,0); 
    gpio_direction_input(gpiod_ts_miso); 
    gpio_direction_output(gpiod_ts_clk,0); 
#endif
	
    ret = gpio_request(GPIO_LCD_RESET,"LcdRst");
    if(ret)
    ...
    printk("==> request LCD gpio OK\n"); 
    
    return 0;
    
#if 1  
    par->gpio.reset = devm_gpiod_get_optional(par->info->device,"reset", GPIOD_OUT_HIGH);
    if(par->gpio.reset)
       printk("==> request LCD reset OK\n"); 
    else
    {
       printk("==> request LCD reset failed\n");  
       return 0;
    ...
    par->gpio.dc = devm_gpiod_get_optional(par->info->device,"dc", GPIOD_OUT_HIGH);
     if(par->gpio.dc)
       printk("==> request LCD dc OK\n"); 
    else{
       printk("==> request LCD cc failed\n"); 
       return 0;
    }
#endif
	...
	if (ret)
        ...
        printk("==> request gpio of LCD reset OK , par->gpio.reset = %d\n",par->gpio.reset);
	...
	if (ret)
        ...
        printk("==> request gpio of LCD dc OK\n");
...
static void fbtft_reset(struct fbtft_par *par)
{
    ...
    __gpio_set_value(GPIO_LCD_RESET,0);
	usleep_range(20, 40);
	//gpiod_set_value_cansleep(par->gpio.reset, 0);
    __gpio_set_value(GPIO_LCD_RESET,1);
	msleep(120);
}
			      ...
			      size_t count, loff_t *ppos)
{
   ...
   // printk("==> fbtft_fb_write: count=%zd, ppos=%llu\n",count, *ppos);
...
static int fbtft_verify_gpios(struct fbtft_par *par)
{
   ...
   // printk("==> pdata->display.buswidth = %d, par->startbyte = %d ,par->gpio.dc = %d\n",pdata->display.buswidth,par->startbyte,par->gpio.dc);
...
static struct fbtft_platform_data *fbtft_properties_read(struct device *dev)
{
   ...
   // printk("==> fbtft_properties_read enter\n");
		       ...
		       struct platform_device *pdev)
{
   ...
   // printk("==> fbtft_probe_common enter\n");
