...
static s32 de_enhance_apply_core(u32 disp)
{
	...
	for (chn = 0; chn < chno; chn++) {
		...
		force_vep = g_para[disp][chn]->force_yuv;
		...
		if (g_para[disp][chn]->flags & (other_dirty |
			...
			de_fce_info2para(disp, chn, force_vep ? 0 : fmt, dev_type,
		...
		if (g_para[disp][chn]->flags &
			...
			if (g_cfg->peak2d_exist[disp][chn])
				de_vsu_set_sharp_para(disp, chn, fmt,
					...
					g_para[disp][chn]->bypass);
			de_peak_info2para(disp, chn, force_vep ? 0 : fmt, dev_type,
		...
		if (g_para[disp][chn]->flags &
			...
			de_lti_info2para(disp, chn, force_vep ? 0 : fmt, dev_type,
		...
		if (g_para[disp][chn]->flags &
			...
			de_fcc_info2para(disp, chn, force_vep ? 0 : fmt, dev_type,
		...
		if (g_para[disp][chn]->flags &
		    (ENH_FORMAT_DIRTY | ENH_ENABLE_DIRTY | ENH_MODE_DIRTY |
		     ENH_SAT_DIRTY | ENH_BYPASS_DIRTY))
			de_bls_info2para(disp, chn, force_vep ? 0 : fmt, dev_type,
			   ...
			   struct disp_enhance_config *config)
{
	...
	for (chn = 0; chn < chno; chn++) {
		...
		if (config[0].flags & ENH_FORCE_FMT_DIRTY) {
			g_para[disp][chn]->force_yuv =
						config[0].info.force_fmt;
		}
