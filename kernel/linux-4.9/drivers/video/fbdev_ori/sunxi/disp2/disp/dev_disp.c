...
static ssize_t disp_enhance_force_fmt_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num_screens = 2;
	struct disp_manager *mgr = NULL;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->get_force_fmt)
			value = enhance->get_force_fmt(enhance);
	}

	return sprintf(buf, "%d\n", value);
}

static ssize_t disp_enhance_force_fmt_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int err;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->set_contrast) {
			enhance->set_force_fmt(enhance, value);
		}
		if (enhance && enhance->set_mode) {
			enhance->set_mode(enhance, g_enhance_mode ? 0 : 1);
			enhance->set_mode(enhance, g_enhance_mode);
...
static DEVICE_ATTR(enhance_force_fmt, 0660,
	disp_enhance_force_fmt_show, disp_enhance_force_fmt_store);
...
static struct attribute *disp_attributes[] = {
	...
	&dev_attr_enhance_force_fmt.attr,
