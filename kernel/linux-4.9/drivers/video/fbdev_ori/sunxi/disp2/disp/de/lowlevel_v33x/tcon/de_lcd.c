...
s32 tcon0_dsi_clk_enable(u32 sel, u32 en)
{
	...
	if (sel == 0)
		lcd_top[0]->tcon_clk_gate.bits.dsi0_clk_gate = en;
	else if (sel == 1)
		lcd_top[0]->tcon_clk_gate.bits.dsi1_clk_gate = en;
...
s32 tcon_de_attach(u32 tcon_index, u32 de_index)
{
	...
	printk("%s:de=%d, tcon=%d", __func__, de_index, tcon_index);
	if (de_index == 0) {
#if defined(CONFIG_ARCH_SUN50IW6)
		...
		if (lcd_top[0]->tcon_de_perh.bits.de_port1_perh
				== tcon_index) {
...
s32 tcon_init(u32 sel)
{
	...
	lcd_dev[sel]->tcon1_ctl.bits.tcon1_en = 0; /*FIXME:this reg is nolonger
						     exist*/
	...
	lcd_dev[sel]->tv_data_io_pol0.dwval = 0x0;
	lcd_dev[sel]->tv_data_io_pol1.dwval = 0x0;
	lcd_dev[sel]->tv_data_io_tri0.dwval = 0x0;
	lcd_dev[sel]->tv_data_io_tri1.dwval = 0x0;
	lcd_dev[sel]->pixel_depth_mode.dwval = 0x0;
...
{
	return lcd_dev[sel]->tcon_gctl.bits.pad_sel;
}
...
static s32 tcon0_cfg_mode_auto(u32 sel, struct disp_panel_para *panel, u32 de_use_rcq)
{
	s32 start_delay;
		...
		if (panel->lcd_interlace) {
			...
			lcd_dev[sel]->tcon0_basic2.bits.vt =
			    (panel->lcd_hv_syuv_fdly ==
			     LCD_HV_SRGB_FDLY_2LINE) ? 625 : 525;
			start_delay = panel->lcd_vt / 2 - panel->lcd_y / 2 - 10;
		} else {
			lcd_dev[sel]->tcon0_basic0.bits.y = panel->lcd_y - 1;
			lcd_dev[sel]->tcon0_basic2.bits.vt =
			    (panel->lcd_hv_syuv_fdly ==
			     LCD_HV_SRGB_FDLY_2LINE) ? 1250 : 1050;
		}
	...
	__inf("sel=%d, de_use_rcq=%d\n", sel, de_use_rcq);
	if (de_use_rcq)
		lcd_dev[sel]->tcon0_ctl.bits.start_delay = 1;
	else
...
s32 tcon0_cfg(u32 sel, struct disp_panel_para *panel, u32 de_use_rcq)
{
	if ((panel->lcd_if == LCD_IF_HV) || (panel->lcd_if == LCD_IF_EXT_DSI)) {
		...
		tcon0_cfg_mode_auto(sel, panel, de_use_rcq);
#if defined(HAVE_DEVICE_COMMON_MODULE)
		rgb_src_sel(sel);
#endif
