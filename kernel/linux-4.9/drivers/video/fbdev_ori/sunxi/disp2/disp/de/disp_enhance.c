...
static s32 disp_enhance_set_force_fmt(struct disp_enhance *enhance, u32 value)
{
	unsigned long flags;
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	spin_lock_irqsave(&enhance_data_lock, flags);
	enhancep->config.info.force_fmt = value ? 1 : 0;
	enhancep->config.flags |= ENH_FORCE_FMT_DIRTY;
	spin_unlock_irqrestore(&enhance_data_lock, flags);
...
static s32 disp_enhance_get_force_fmt(struct disp_enhance *enhance)
{
	struct disp_enhance_private_data *enhancep =
	    disp_enhance_get_priv(enhance);
	...
	return enhancep->config.info.force_fmt;
}
...
s32 disp_init_enhance(struct disp_bsp_init_para *para)
{
	...
	for (disp = 0; disp < num_enhances; disp++) {
		...
		enhance->set_force_fmt = disp_enhance_set_force_fmt;
		enhance->get_force_fmt = disp_enhance_get_force_fmt;
