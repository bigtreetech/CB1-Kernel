				 ...
				 struct disp_init_para *init_para)
{
    ...
    GetHdmiParam(&hdmi_width,&hdmi_height,&hdmi_mode);
        
    init_para->fb_width[0] = hdmi_width;  
    init_para->fb_height[0] = hdmi_height;
    init_para->output_mode[0] = hdmi_mode;
    
    printk("\n=====> kernel get hdmi = %dx%d mode = %d\n\n",hdmi_width,hdmi_height,hdmi_mode);

#if 0
	printk("====display init para begin====\n");
	printk("b_init:%d\n", init_para->b_init);
	printk("disp_mode:%d\n\n", init_para->disp_mode);
	for (i = 0; i < DISP_SCREEN_NUM; i++) {
		printk("output_type[%d]:%d\n", i, init_para->output_type[i]);
		printk("output_mode[%d]:%d\n", i, init_para->output_mode[i]);
	}
	for (i = 0; i < DISP_SCREEN_NUM; i++) {
		printk("buffer_num[%d]:%d\n", i, init_para->buffer_num[i]);
		printk("format[%d]:%d\n", i, init_para->format[i]);
		printk("fb_width[%d]:%d\n", i, init_para->fb_width[i]);
		printk("fb_height[%d]:%d\n", i, init_para->fb_height[i]);
	}
	printk("====display init para end====\n");
#endif
...
static s32 disp_init(struct platform_device *pdev)
{
   ...
   // printk("===> boot_disp = %d\n",value);
    ...
    output_mode =  hdmi_mode; //kandy
    
   // printk("===> output_type = %d , output_mode = %d\n",output_type,output_mode);
