...
static int mmc_rescan_try_freq(struct mmc_host *host, unsigned freq)
{
	...
	if (!(host->caps2 & MMC_CAP2_NO_SDIO))
        ...
        /*
        else if(!wifi_on_flag) 
        {
            for(i=0;i<5;i++)
            {
               msleep(200);
               printk("===> try mmc_attach_sdio %d times\n",i);
               if( !mmc_attach_sdio(host) ) 
               {
                   return 0;
                   break;
...
void mmc_rescan(struct work_struct *work)
{
	...
	int i;
     ...
     printk("==>mmc1_skip_counter = %d\n",mmc1_skip_counter);
    
    if(mmc1_skip_counter == 2) 
    {
       char *p = mmc_hostname(host);
       
       printk("==>mmc_hostname = %s\n",p);
       
       if( strstr( p,"mmc1") !=NULL )
       {
          printk("==> mmc1 mmc_rescan exit\n");
          return ;
       }
       else
          printk("==> mmc_rescan: is not mmc1\n"); 
    }   
