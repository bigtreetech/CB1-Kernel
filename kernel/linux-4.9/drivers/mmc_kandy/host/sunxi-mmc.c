				      ...
				      struct mmc_ios *ios)
{
    ...
    printk("==> sunxi_mmc_clk_set_rate_for_sdmmc0 \n");
...
static void sunxi_mmc_dump_errinfo(struct sunxi_mmc_host *host)
{
    ...
    {
        run_once_counter++;
        printk("==> run_once_counter = %d\n",run_once_counter);
        if(run_once_counter == 1)
        {
          mmc1_error_flag = 1;
          printk("==> mmc1 scan error\n");
        }
    }
...
static irqreturn_t sunxi_mmc_irq(int irq, void *dev_id)
{
	...
	if (mrq) {
        ...
        {
			finalize = true;
            printk("==> sunxi_mmc_irq SDXC_INTERRUPT_ERROR_BIT\n");
        }
		else if ((host->int_sum & SDXC_INTERRUPT_DONE_BIT) &&
			 !host->wait_dma)
        {
			finalize = true;
           // printk("==> sunxi_mmc_irq SDXC_INTERRUPT_DONE_BIT\n");
        }
	}
	
    if( host->mmc->index == 1 )
    { 
       if(!wifi_on_flag) //kandy
       {
         // finalize = true;
          // printk("==> wifi is not opened,set IRQ error\n");
       }
    }	
...
static int mmc1_skip_counter=0;
...
static void sunxi_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	static const char * const timing[] = {
		"UHS-SDR12","LEGACY(SDR12)", "MMC-HS(SDR20)", "SD-HS(SDR25)","UHS-SDR25",
		"UHS-SDR50", "UHS-SDR104", "UHS-DDR50", "MMC-DDR52","MMC-HS200", "MMC-HS400"
	};
    ...
    { 
       if(!wifi_on_flag) //kandy
       {
         //  ios->timing = timing[1];
         //  ios->clock = 400000;
         // printk("==> wifi is not opened,mmc1 sunxi_mmc_set_ios\n");
       }
    }      
...
static int sunxi_mmc_gpio_get_cd(struct mmc_host *mmc)
{
	...
	if (host->present ^ present) {
		...
		printk("----------- sunxi_mmc_gpio_get_cd return 1\n");
		return 1;
	}
       printk("----------- sunxi_mmc_gpio_get_cd return 0\n");
	return 0;
}
...
#ifdef SUNXI_SDMMC0
	if (of_device_is_compatible(np, "allwinner,sun50i-sdmmc0")|| of_device_is_compatible(np, "allwinner,sun8iw10p1-sdmmc0")) 
	{
		host->sunxi_mmc_clk_set_rate =
