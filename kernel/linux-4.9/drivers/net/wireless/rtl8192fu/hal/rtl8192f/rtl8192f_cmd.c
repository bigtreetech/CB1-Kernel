...
#define RTL8192F_MAX_CMD_LEN	7
#define RTL8192F_EX_MESSAGE_BOX_SIZE	4
...
{
	u8 h2c_box_num;
	u8 h2c[RTL8192F_MAX_CMD_LEN + 1] = {0};
	u32	msgbox_addr;
	...
	if (!pCmdBuffer)
		goto exit;
	if (CmdLen > RTL8192F_MAX_CMD_LEN)
		goto exit;
	...
	do {
		...
		msgbox_ex_addr = REG_HMEBOX_EXT0_8192F + (h2c_box_num * RTL8192F_EX_MESSAGE_BOX_SIZE);
		_rtw_memcpy((u8 *)(&h2c_cmd_ex), h2c + 4, RTL8192F_EX_MESSAGE_BOX_SIZE);
		h2c_cmd_ex = le32_to_cpu(h2c_cmd_ex);
		rtw_write32(padapter, msgbox_ex_addr, h2c_cmd_ex);
		/* Write command (byte 0~3) */
		msgbox_addr = REG_HMEBOX_0_8192F + (h2c_box_num * MESSAGE_BOX_SIZE);
...
u8 GetTxBufferRsvdPageNum8192F(_adapter *padapter, bool wowlan)
{
	...
	u8	TxPageBndy = LAST_ENTRY_OF_TX_PKT_BUFFER_8192F;

	rtw_hal_get_def_var(padapter, HAL_DEF_TX_PAGE_BOUNDARY, (u8 *)&TxPageBndy);

	RsvdPageNum = LAST_ENTRY_OF_TX_PKT_BUFFER_8192F - TxPageBndy + 1;

	return RsvdPageNum;
}
void rtl8192f_req_txrpt_cmd(PADAPTER adapter, u8 macid)
{
	u8 u1H2CApReqRptParm[H2C_AP_REQ_TXRPT_LEN] = {0};

	SET_8192F_H2CCMD_TXREP_PARM_STA1(u1H2CApReqRptParm, macid);
	SET_8192F_H2CCMD_TXREP_PARM_STA2(u1H2CApReqRptParm, 0xff);
	SET_8192F_H2CCMD_TXREP_PARM_RTY(u1H2CApReqRptParm, 0x00);
	FillH2CCmd8192F(adapter, H2C_AP_REQ_TXRPT, H2C_AP_REQ_TXRPT_LEN, u1H2CApReqRptParm);
	SET_8192F_H2CCMD_TXREP_PARM_RTY(u1H2CApReqRptParm, 0x02);
	FillH2CCmd8192F(adapter, H2C_AP_REQ_TXRPT, H2C_AP_REQ_TXRPT_LEN, u1H2CApReqRptParm);
}

void rtl8192f_set_FwPwrMode_cmd(PADAPTER padapter, u8 psmode)
{
	u8 smart_ps = 0;
	...
	*/
	...
	SET_8192F_H2CCMD_PWRMODE_PARM_MODE(u1H2CPwrModeParm, (psmode > 0) ? 1 : 0);
	SET_8192F_H2CCMD_PWRMODE_PARM_SMART_PS(u1H2CPwrModeParm, smart_ps);
	SET_8192F_H2CCMD_PWRMODE_PARM_RLBM(u1H2CPwrModeParm, rlbm);
	SET_8192F_H2CCMD_PWRMODE_PARM_BCN_PASS_TIME(u1H2CPwrModeParm, awake_intvl);
	SET_8192F_H2CCMD_PWRMODE_PARM_ALL_QUEUE_UAPSD(u1H2CPwrModeParm, allQueueUAPSD);
	SET_8192F_H2CCMD_PWRMODE_PARM_PWR_STATE(u1H2CPwrModeParm, PowerState);
	...
	FillH2CCmd8192F(padapter, H2C_8192F_SET_PWR_MODE, H2C_PWRMODE_LEN, u1H2CPwrModeParm);
}
...
void rtl8192f_set_FwPsTuneParam_cmd(PADAPTER padapter)
{
	...
	SET_8192F_H2CCMD_PSTUNE_PARM_BCN_TO_LIMIT(u1H2CPsTuneParm, bcn_to_limit);
	SET_8192F_H2CCMD_PSTUNE_PARM_DTIM_TIMEOUT(u1H2CPsTuneParm, dtim_timeout);
	SET_8192F_H2CCMD_PSTUNE_PARM_PS_TIMEOUT(u1H2CPsTuneParm, ps_timeout);
	SET_8192F_H2CCMD_PSTUNE_PARM_ADOPT(u1H2CPsTuneParm, 1);
	SET_8192F_H2CCMD_PSTUNE_PARM_DTIM_PERIOD(u1H2CPsTuneParm, dtim_period);

	RTW_DBG_DUMP("u1H2CPsTuneParm:",
		     u1H2CPsTuneParm, H2C_PSTUNEPARAM_LEN);

	FillH2CCmd8192F(padapter, H2C_8192F_PS_TUNING_PARA, H2C_PSTUNEPARAM_LEN, u1H2CPsTuneParm);
}
...
void rtl8192f_download_rsvd_page(PADAPTER padapter, u8 mstatus)
{
	...
	if(registry_par->la_mode_en == 1) {
		 RTW_INFO("%s():  block download_rsvd_page\n", __func__);
		return;
	}
#endif
...
#ifdef CONFIG_P2P
void rtl8192f_set_p2p_ps_offload_cmd(_adapter *padapter, u8 p2p_ps_state)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
	struct pwrctrl_priv		*pwrpriv = adapter_to_pwrctl(padapter);
	...
	if (psta) {
		p2p_ps_offload->p2p_macid = psta->cmn.mac_id;
	} else {
...
#ifdef CONFIG_P2P_PS_NOA_USE_MACID_SLEEP
		_rtw_memset(p2p_ps_offload, 0, 2);
#else
...
#ifdef CONFIG_P2P_PS_NOA_USE_MACID_SLEEP
			p2p_ps_offload->disable_close_rf = 0;
#ifdef CONFIG_CONCURRENT_MODE
			if (rtw_mi_buddy_check_fwstate(padapter, WIFI_ASOC_STATE))
#endif /* CONFIG_CONCURRENT_MODE */
				p2p_ps_offload->disable_close_rf = 1;
#endif /* CONFIG_P2P_PS_NOA_USE_MACID_SLEEP */
			}
...
#ifdef CONFIG_P2P_PS_NOA_USE_MACID_SLEEP
	FillH2CCmd8192F(padapter, H2C_8192F_P2P_PS_OFFLOAD, 2, (u8 *)p2p_ps_offload);
#else
	FillH2CCmd8192F(padapter, H2C_8192F_P2P_PS_OFFLOAD, 1, (u8 *)p2p_ps_offload);
#endif /* CONFIG_P2P_PS_NOA_USE_MACID_SLEEP */
#endif
