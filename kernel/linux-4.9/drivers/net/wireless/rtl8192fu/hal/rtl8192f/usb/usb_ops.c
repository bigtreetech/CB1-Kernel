...
void rtl8192fu_interrupt_handler(_adapter *padapter, u16 pkt_len, u8 *pbuf)
{
	...
	/*only for BCN_0*/
	if (pHalData->IntArray[0] & IMR_BCNDMAINT0_8192F)
#endif
	...
	if (pHalData->IntArray[1] & IMR_TXERR_8192F)
		RTW_INFO("===> %s Tx Error Flag Interrupt Status\n", __func__);
	if (pHalData->IntArray[1] & IMR_RXERR_8192F)
		RTW_INFO("===> %s Rx Error Flag INT Status\n", __func__);
	if (pHalData->IntArray[1] & IMR_TXFOVW_8192F)
		RTW_INFO("===> %s Transmit FIFO Overflow\n", __func__);
	if (pHalData->IntArray[1] & IMR_RXFOVW_8192F)
...
int recvbuf2recvframe(PADAPTER padapter, void *ptr)
{
	...
	pkt_cnt = GET_RX_STATUS_DESC_USB_AGG_PKTNUM_8192F(pbuf);
#endif
	...
	do {
		...
		rtl8192f_query_rx_desc_status(precvframe, pbuf);
...
void rtl8192fu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8192fu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
{
	pdvobj->HardwareType = HARDWARE_TYPE_RTL8192FU;
	RTW_INFO("CHIP TYPE: RTL8192FU\n");
}
