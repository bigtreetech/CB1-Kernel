...
#include <hal_com.h>
#include "hal8192f_fw.h"


#ifndef CONFIG_DLFW_TXPKT
#define DL_FW_MAX 15
#else
#define FW_DOWNLOAD_SIZE_8192F 8192
#endif
...
)
{
	...
	if(enable)
	{
		...
		PlatformEFIOWrite1Byte(padapter, REG_8051FW_CTRL_8192F, 0x05);	

		// Clear Rom DL enable
		tmp = PlatformEFIORead1Byte(padapter, REG_8051FW_CTRL_8192F+2);
		PlatformEFIOWrite1Byte(padapter, REG_8051FW_CTRL_8192F+2, tmp&0xf7);
	}
	else
	{
		// MCU firmware download enable.
		tmp = PlatformEFIORead1Byte(padapter, REG_8051FW_CTRL_8192F);
		PlatformEFIOWrite1Byte(padapter, REG_8051FW_CTRL_8192F, tmp&0xfe);
	}
}
...
)
{
	...
	if (blockCount_p1) {
		RTW_DBG("%s:_BlockWrite: [P1] buffSize(%d) blockSize_p1(%d)blockCount_p1(%d) remainSize_p1(%d)\n",
			__FUNCTION__, buffSize, blockSize_p1, blockCount_p1, remainSize_p1);
	}

	for (i = 0; i < blockCount_p1; i++) {
#ifdef CONFIG_USB_HCI
		ret = rtw_writeN(padapter, (FW_8192F_START_ADDRESS + i * blockSize_p1), blockSize_p1, (bufferPtr + i * blockSize_p1));
#else
		ret = rtw_write32(padapter, (FW_8192F_START_ADDRESS + i * blockSize_p1), le32_to_cpu(*((u32 *)(bufferPtr + i * blockSize_p1))));
#endif
		if (ret == _FAIL) {
	...
	if (remainSize_p1) {
		...
		case 1:
			remainFW[0] = *(p);
			ret = rtw_write32(padapter, (FW_8192F_START_ADDRESS + blockCount_p1 * blockSize_p1),
	...
	if (remainSize_p1) {
		...
		if (blockCount_p2) {
			RTW_DBG("%s:_BlockWrite: [P2] buffSize_p2(%d) blockSize_p2(%d) blockCount_p2(%d) remainSize_p2(%d)\n",
				__FUNCTION__, (buffSize-offset), blockSize_p2, blockCount_p2, remainSize_p2);
		}
#ifdef CONFIG_USB_HCI
		for (i = 0; i < blockCount_p2; i++) {
			ret = rtw_writeN(padapter, (FW_8192F_START_ADDRESS + offset + i * blockSize_p2), blockSize_p2, (bufferPtr + offset + i * blockSize_p2));
	...
	RTW_DBG("%s:_BlockWrite: [P3] buffSize_p3(%d) blockSize_p3(%d) blockCount_p3(%d)\n",
			 __FUNCTION__, (buffSize-offset), blockSize_p3, blockCount_p3);
		for (i = 0 ; i < blockCount_p3 ; i++) {
			ret = rtw_write8(padapter, (FW_8192F_START_ADDRESS + offset + i), *(bufferPtr + offset + i));
...
void _8051Reset8192F(PADAPTER padapter)
{
	...
	io_rst = rtw_read8(padapter, REG_RSV_CTRL + 1) & BIT(0);
	RTW_DBG("0x1c[8]: %x, Reset IO wrapper %s\n", io_rst, io_rst?"Fail":"Success");
	...
	u1bTmp = PlatformEFIORead1Byte(padapter, REG_RSV_CTRL);
	PlatformEFIOWrite1Byte(padapter, REG_RSV_CTRL, (u1bTmp&0x02));
	
	// Suggest by Gimmy Song. RESET I/O Wrapper
	
	// 0x1c[8] = 0
	u1bTmp = PlatformEFIORead1Byte(padapter, REG_RSV_CTRL+1);
	PlatformEFIOWrite1Byte(padapter, REG_RSV_CTRL+1, u1bTmp&(~BIT0));
	u1bRd = PlatformEFIORead1Byte(padapter, REG_RSV_CTRL+1) & BIT0;
	RT_TRACE(_module_hal_init_c_, _drv_info_, ("0x1c[8]: %x, Reset IO wrapper %s\n", u1bRd, u1bRd?"Fail":"Success"));


    // 0x3[2] = 0, RESET CPU
	u1bTmp = PlatformEFIORead1Byte(padapter, REG_SYS_FUNC_EN+1);
	PlatformEFIOWrite1Byte(padapter, REG_SYS_FUNC_EN+1, u1bTmp&(~BIT2));
#endif
}
...
static s32 _FWFreeToGo(_adapter *adapter, u32 min_cnt, u32 timeout_ms)
{
	...
	u32 value_expected = ( WINTINI_RDY|RAM_DL_SEL);/*from @brian_zhang */
...
void rtl8192f_FirmwareSelfReset(PADAPTER padapter)
{
	...
	u8 FwBuffer[FW_8192F_SIZE];
#endif /* CONFIG_FILE_FWIMG */

#ifdef CONFIG_MP_INCLUDED
int _WriteBTFWtoTxPktBuf8192F(
			PADAPTER	Adapter,
			void			*buffer,
			u32			FwBufLen,
			u8			times
)
{
	...
	rtStatus =	InitLLTTable8192FS(Adapter, txpktbuf_bndy);
	if (RT_STATUS_SUCCESS != rtStatus) {
		RTW_INFO("_CheckWLANFwPatchBTFwReady_8192F(): Failed to init LLT!\n");
		return RT_STATUS_FAILURE;
	}

	/* Init Tx boundary. */
	PlatformEFIOWrite1Byte(Adapter, REG_DWBCN0_CTRL_8192F + 1, (u8)txpktbuf_bndy);
#endif
...
exit:
	...
	if (pGenBufReservedPagePacket) {
		RTW_INFO("_WriteBTFWtoTxPktBuf8192F => rtw_mfree pGenBufReservedPagePacket!\n");
...
)
{
	...
	SET_8192F_H2CCMD_BT_FW_PATCH_SIZE(u1BTFwPatchParm, FwSize);
	SET_8192F_H2CCMD_BT_FW_PATCH_ADDR0(u1BTFwPatchParm, addr0);
	SET_8192F_H2CCMD_BT_FW_PATCH_ADDR1(u1BTFwPatchParm, addr1);
	SET_8192F_H2CCMD_BT_FW_PATCH_ADDR2(u1BTFwPatchParm, addr2);
	SET_8192F_H2CCMD_BT_FW_PATCH_ADDR3(u1BTFwPatchParm, addr3);

	FillH2CCmd8192F(Adapter, H2C_8192F_BT_FW_PATCH, H2C_BT_FW_PATCH_LEN, u1BTFwPatchParm);
...
)
{
	...
	SET_8192F_H2CCMD_BT_PWR_IDX(u1BTPwrIdxParm, PwrIdx);


	FillH2CCmd8192F(Adapter, H2C_8192F_FORCE_BT_TXPWR, H2C_FORCE_BT_TXPWR_LEN, u1BTPwrIdxParm);
}
...
)
{
	...
	/* --------------------------------------------------------- */
	do {
		u1bTmp = PlatformEFIORead1Byte(Adapter, REG_HMEBOX_DBG_0_8192F);
 ...
 * into four parts to transfer. The last parameter of _WriteBTFWtoTxPktBuf8192F is used to indicate
...
s32 FirmwareDownloadBT(PADAPTER padapter, PRT_MP_FIRMWARE pFirmware)
{
	...
	if (rtw_is_file_readable(rtw_fw_mp_bt_file_path) == _TRUE) {
		...
		rtStatus = rtw_retrieve_from_file(rtw_fw_mp_bt_file_path, FwBuffer, FW_8192F_SIZE);
...
#endif /* CONFIG_FILE_FWIMG */
	{
		...
		pBTFirmwareBuf = (u8 *)Rtl8192FFwBTImgArray;
		BTFirmwareLen = Rtl8192FFwBTImgArrayLength;
	...
	for (i = (download_time - 1); i >= 0; i--) {
		if (i == (download_time - 1)) {
			rtStatus = _WriteBTFWtoTxPktBuf8192F(padapter, pBTFirmwareBuf + (4096 * i), (BTFirmwareLen - (4096 * i)), 1);
			RTW_INFO("%s: start %d, len %d, time 1\n", __FUNCTION__, 4096 * i, BTFirmwareLen - (4096 * i));
		} else {
			rtStatus = _WriteBTFWtoTxPktBuf8192F(padapter, pBTFirmwareBuf + (4096 * i), 4096, (download_time - i));
...
u8 send_fw_packet(PADAPTER padapter, u8 *pRam_code, u32 length)
{
	...
	if (FW_DOWNLOAD_SIZE_8192F == length)
		length--;
	...
	SET_TX_DESC_QUEUE_SEL_8192F(pTmp_buffer, QSLT_BEACON);
	SET_TX_DESC_PKT_SIZE_8192F(pTmp_buffer, modify_ram_size - TXDESC_SIZE);
	SET_TX_DESC_OFFSET_8192F(pTmp_buffer, TXDESC_SIZE);
	...
	do {
		...
		value8 = rtw_read8(padapter, REG_DWBCN0_CTRL_8192F + 2);
		counter--;
	} while (0 == (value8 & BIT(0)));

	rtw_write8(padapter, REG_DWBCN0_CTRL_8192F + 2, value8);
...
s32 rtl8192f_FirmwareDownload(PADAPTER padapter, BOOLEAN  bUsedWoWLANFw)
{
	...
	PRT_FIRMWARE_8192F	pFirmware = NULL;
	PRT_8192F_FIRMWARE_HDR		pFwHdr = NULL;
	u8			*pFirmwareBuf;
	u32			FirmwareLen;
#ifdef CONFIG_FILE_FWIMG
	u8 *fwfilepath;
	...
	pFirmware = (PRT_FIRMWARE_8192F)rtw_zmalloc(sizeof(RT_FIRMWARE_8192F));
...
#ifdef CONFIG_FILE_FWIMG
		rtStatus = rtw_retrieve_from_file(fwfilepath, FwBuffer, FW_8192F_SIZE);
	...
	case FW_SOURCE_HEADER_FILE:
		if (bUsedWoWLANFw) {
			...
			if (pwrpriv->wowlan_mode) {
				pFirmware->szFwBuffer = array_mp_8192f_fw_wowlan;
				pFirmware->ulFwLength = array_length_mp_8192f_fw_wowlan;
			...
			if (pwrpriv->wowlan_ap_mode) {
				pFirmware->szFwBuffer = array_mp_8192f_fw_ap;
				pFirmware->ulFwLength = array_length_mp_8192f_fw_ap;
				RTW_INFO(" ===> %s fw: %s, size: %d\n",
					 __FUNCTION__, "AP_WoWLAN", pFirmware->ulFwLength);
			}
	#endif /* CONFIG_AP_WOWLAN */
		} else {
			pFirmware->szFwBuffer = array_mp_8192f_fw_nic;
			pFirmware->ulFwLength = array_length_mp_8192f_fw_nic;
			RTW_INFO("%s fw: %s, size: %d\n", __FUNCTION__, "FW_NIC", pFirmware->ulFwLength);
		}
		break;
	}

	if ((pFirmware->ulFwLength - 32) > FW_8192F_SIZE) {
		rtStatus = _FAIL;
		RTW_ERR("Firmware size:%u exceed %u\n",
			pFirmware->ulFwLength, FW_8192F_SIZE);
		goto exit;
	}
	...
	pFwHdr = (PRT_8192F_FIRMWARE_HDR)pFirmwareBuf;
	...
	if (IS_FW_HEADER_EXIST_8192F(pFwHdr)) {
	...
	if ((rtw_read8(padapter, REG_MCUFWDL) & MCUFWDL_RDY) == 0) {
		...
		*/
		...
		rtw_write16(padapter, REG_DWBCN0_CTRL_8192F + 1, BIT(8));
		...
		while (0 != pkt_size_tmp) {
			if (pkt_size_tmp >= FW_DOWNLOAD_SIZE_8192F) {
				send_pkt_size = FW_DOWNLOAD_SIZE_8192F;
...
exit:
	if (pFirmware)
		rtw_mfree((u8 *)pFirmware, sizeof(RT_FIRMWARE_8192F));

	rtl8192f_InitializeFirmwareVars(padapter);
	...
	u8			bPseudoTest)
{
	...
	case TYPE_EFUSE_MAX_SECTION: {
		...
		if (efuseType == EFUSE_WIFI)
			*pMax_section = EFUSE_MAX_SECTION_8192F;
		else
	...
	case TYPE_EFUSE_REAL_CONTENT_LEN: {
		...
		if (efuseType == EFUSE_WIFI)
			*pu2Tmp = EFUSE_REAL_CONTENT_LEN_8192F;
		else
	...
	case TYPE_AVAILABLE_EFUSE_BYTES_BANK: {
		...
		if (efuseType == EFUSE_WIFI)
			*pu2Tmp = (EFUSE_REAL_CONTENT_LEN_8192F - EFUSE_OOB_PROTECT_BYTES);
		else
	...
	u8			bPseudoTest)
{
	...
	while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
		...
		if (offset < EFUSE_MAX_SECTION_8192F) {
			u16 addr;
...
)
{
	...
	for (bank = 1; bank < 3; bank++) { /* 8192F Max bake 0~2 */
		...
		while (AVAILABLE_EFUSE_ADDR(eFuse_Addr)) {
			...
			if (efuseHeader == 0xFF)
				break;
			RTW_INFO("%s: efuse[%#X]=0x%02x (header)\n", __FUNCTION__, (((bank - 1) * EFUSE_REAL_CONTENT_LEN_8192F) + eFuse_Addr - 1), efuseHeader);
			...
			if (EXT_HEADER(efuseHeader)) {	/* extended header */
				...
				/* ReadEFuseByte(padapter, eFuse_Addr++, &efuseExtHdr, bPseudoTest); */
				efuse_OneByteRead(padapter, eFuse_Addr++, &efuseExtHdr, bPseudoTest);
				RTW_INFO("%s: efuse[%#X]=0x%02x (ext header)\n", __FUNCTION__, (((bank - 1) * EFUSE_REAL_CONTENT_LEN_8192F) + eFuse_Addr - 1), efuseExtHdr);
				if (ALL_WORDS_DISABLED(efuseExtHdr))
					continue;
...
static void read_chip_version_8192f(PADAPTER padapter)
{
	u32				value32;
	...
	pHalData->version_id.ICType = CHIP_8192F;
	pHalData->version_id.ChipType = ((value32 & RTL_ID) ? TEST_CHIP : NORMAL_CHIP);
	pHalData->version_id.RFType = RF_TYPE_2T2R ;
...
{
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
	u16 val16, wlan_act_mask_ctrl = 0;
	u8 val8;
	...
	wlan_act_mask_ctrl |= EN_PORT_0_FUNCTION;
#endif

#ifdef CONFIG_CONCURRENT_MODE
	val16 |= (EN_BCN_FUNCTION << 8);
	wlan_act_mask_ctrl |= EN_PORT_1_FUNCTION;
#endif
	rtw_write16(padapter, REG_BCN_CTRL, val16);
	rtw_write16(padapter, REG_WLAN_ACT_MASK_CTRL_1, rtw_read16(padapter, 
				REG_WLAN_ACT_MASK_CTRL_1) | wlan_act_mask_ctrl);
	...
	if (check_fwstate(&padapter->mlmepriv, WIFI_STATION_STATE) == _FALSE)
		rtw_write8(padapter, REG_DRVERLYINT, DRIVER_EARLY_INT_TIME_8192F); /* 5ms */
	rtw_write8(padapter, REG_BCNDMATIM, BCN_DMA_ATIME_INT_TIME_8192F); /* 2ms */
...
void _InitMacAPLLSetting_8192F(PADAPTER Adapter)
{
	...
	RegValue = rtw_read16(Adapter, REG_AFE_CTRL_4_8192F);
	RegValue |= BIT(4);
	RegValue |= BIT(15);
	rtw_write16(Adapter, REG_AFE_CTRL_4_8192F, RegValue);
#ifdef CONFIG_MP_INCLUDED
	if (Adapter->registrypriv.mp_mode == 1) {
		rtw_write32(Adapter, REG_MACID_8192F, 0x87654321);
...
static void rtl8192f_SetBeaconRelatedRegisters(PADAPTER padapter)
{
	u8 val8;
	u32 value32;
	PHAL_DATA_TYPE pHalData = GET_HAL_DATA(padapter);
...
void hal_notch_filter_8192f(_adapter *adapter, bool enable)
{
	if (enable) {
...
void rtl8192f_fill_fake_txdesc(
	PADAPTER	padapter,
	u8			*pDesc,
	u32			BufferLen,
	u8			IsPsPoll,
	...
	u8			bDataFrame)
{
	...
	SET_TX_DESC_OFFSET_8192F(pDesc, 0x28); /* Offset = 32 */

	SET_TX_DESC_PKT_SIZE_8192F(pDesc, BufferLen); /* Buffer size + command header */
	SET_TX_DESC_QUEUE_SEL_8192F(pDesc, QSLT_MGNT); /* Fixed queue of Mgnt queue */

	/* Set NAVUSEHDR to prevent Ps-poll AId filed to be changed to error vlaue by Hw. */
	if (_TRUE == IsPsPoll)
		SET_TX_DESC_NAV_USE_HDR_8192F(pDesc, 1);
	else {
		SET_TX_DESC_HWSEQ_EN_8192F(pDesc, 1); /* Hw set sequence number */
		SET_TX_DESC_HWSEQ_SEL_8192F(pDesc, 0);
	}

	if (_TRUE == IsBTQosNull)
		SET_TX_DESC_BT_INT_8192F(pDesc, 1);

	SET_TX_DESC_USE_RATE_8192F(pDesc, 1); /* use data rate which is set by Sw */

	SET_TX_DESC_TX_RATE_8192F(pDesc, DESC8192F_RATE1M);
	...
	if (_TRUE == bDataFrame) {
		...
		switch (EncAlg) {
		case _NO_PRIVACY_:
			SET_TX_DESC_SEC_TYPE_8192F(pDesc, 0x0);
			break;
		case _WEP40_:
		case _WEP104_:
		case _TKIP_:
			SET_TX_DESC_SEC_TYPE_8192F(pDesc, 0x1);
			break;
		case _SMS4_:
			SET_TX_DESC_SEC_TYPE_8192F(pDesc, 0x2);
			break;
		case _AES_:
			SET_TX_DESC_SEC_TYPE_8192F(pDesc, 0x3);
			break;
		default:
			SET_TX_DESC_SEC_TYPE_8192F(pDesc, 0x0);
			break;
	 ...
	 */
	rtl8192f_cal_txdesc_chksum((struct tx_desc *)pDesc);
#endif
}
...
void init_hal_spec_8192f(_adapter *adapter)
{
	...
	hal_spec->ic_name = "rtl8192f";
	hal_spec->macid_num = 128;
	hal_spec->sec_cam_ent_num = 64;
	hal_spec->sec_cap = SEC_CAP_CHK_BMC | SEC_CAP_CHK_WRITE_CAM_NEW_RULE;
	hal_spec->wow_cap = WOW_CAP_TKIP_OL;
	hal_spec->macid_cap = MACID_DROP_INDIRECT;
	hal_spec->macid_txrpt = 0x8900;
	hal_spec->macid_txrpt_pgsz = 24;
...
void _DisableRFAFEAndResetBB8192F(PADAPTER padapter)
{
	/**************************************
...
{
	_DisableRFAFEAndResetBB8192F(padapter);
}

void _ResetDigitalProcedure1_8192F(PADAPTER padapter, BOOLEAN bWithoutHWSM)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
...
{
	_ResetDigitalProcedure1_8192F(padapter, bWithoutHWSM);
}
	...
	if (_FALSE == pHalData->bautoload_fail_flag) {
		...
		*		if (IS_BOOT_FROM_EEPROM(padapter)) */
		if (_TRUE == pHalData->EepromOrEfuse) {
			...
			for (i = 0; i < HWSET_MAX_SIZE_8192F; i += 2) {
...
void
Hal_EfuseParseTxPowerInfo_8192F(
		PADAPTER		padapter,
		u8			*PROMContent,
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->EEPROMRegulatory = (PROMContent[EEPROM_RF_BOARD_OPTION_8192F] & 0x7);	/* bit0~2 */
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8192F] == 0xFF)
...
)
{
	...
	if (!AutoloadFail) {
		pHalData->InterfaceSel = (PROMContent[EEPROM_RF_BOARD_OPTION_8192F] & 0xE0) >> 5;
		if (PROMContent[EEPROM_RF_BOARD_OPTION_8192F] == 0xFF)
...
#ifdef CONFIG_BT_COEXIST
void
Hal_EfuseParseBTCoexistInfo_8192F(
		PADAPTER padapter,
		u8 *hwinfo,
		BOOLEAN AutoLoadFail
)
{
	...
	if (!AutoLoadFail) {
		if ((hwinfo[EEPROM_RF_BOARD_OPTION_8192F] != 0xFF)) {
			/* 0xc1[7:5] = 0x01 */
			if (((hwinfo[EEPROM_RF_BOARD_OPTION_8192F] & 0xe0) >> 5) == 0x01)
		...
		pHalData->EEPROMBluetoothType = BT_RTL8192F;

		tempval = hwinfo[EEPROM_RF_BT_SETTING_8192F];
		if (tempval != 0xFF) {
			/* 0:Ant_x2, 1:Ant_x1 */
...
)
{
	...
	if (!AutoLoadFail)
		pHalData->EEPROMVersion = hwinfo[EEPROM_VERSION_8192F];
	else
...
)
{
	...
	/* _rtw_memcpy(pHalData->adjuseVoltageVal, &hwinfo[EEPROM_Voltage_ADDR_8192F], 1); */
	RTW_INFO("%s hwinfo[EEPROM_Voltage_ADDR_8192F] =%02x\n", __func__, hwinfo[EEPROM_Voltage_ADDR_8192F]);
	pHalData->adjuseVoltageVal = (hwinfo[EEPROM_Voltage_ADDR_8192F] & 0xf0) >> 4;
	RTW_INFO("%s pHalData->adjuseVoltageVal =%x\n", __func__, pHalData->adjuseVoltageVal);
}

void
Hal_EfuseParseChnlPlan_8192F(
		PADAPTER		padapter,
		u8			*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	hal_com_config_channel_plan(
		padapter
		, hwinfo ? &hwinfo[EEPROM_COUNTRY_CODE_8192F] : NULL
		, hwinfo ? hwinfo[EEPROM_ChannelPlan_8192F] : 0xFF
...
void
Hal_EfuseParseCustomerID_8192F(
		PADAPTER		padapter,
		u8			*hwinfo,
		BOOLEAN			AutoLoadFail
)
{
	...
	if (!AutoLoadFail)
		pHalData->EEPROMCustomerID = hwinfo[EEPROM_CustomID_8192F];
	else
		pHalData->EEPROMCustomerID = 0;
}

void
Hal_EfuseParseAntennaDiversity_8192F(
		PADAPTER		pAdapter,
		u8				*hwinfo,
		BOOLEAN			AutoLoadFail
)
...
)
{
	...
	if (!AutoLoadFail) {
		pHalData->crystal_cap = hwinfo[EEPROM_XTAL_8192F];
		if (pHalData->crystal_cap == 0xFF)
			pHalData->crystal_cap = EEPROM_Default_CrystalCap_8192F;	   /* what value should 8812 set? */
	} else
		pHalData->crystal_cap = EEPROM_Default_CrystalCap_8192F;
}


void
Hal_EfuseParseThermalMeter_8192F(
	PADAPTER	padapter,
	u8			*PROMContent,
	u8			AutoLoadFail
)
{
	...
	if (_FALSE == AutoLoadFail)
		pHalData->eeprom_thermal_meter = PROMContent[EEPROM_THERMAL_METER_8192F];
	else
		pHalData->eeprom_thermal_meter = EEPROM_Default_ThermalMeter_8192F;
...
void fill_txdesc_force_bmc_camid(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	if ((pattrib->encrypt > 0) && (!pattrib->bswenc)
		...
		SET_TX_DESC_EN_DESC_ID_8192F(ptxdesc, 1);
		SET_TX_DESC_MACID_8192F(ptxdesc, pattrib->bmc_camid);
	}
}
...
void fill_txdesc_bmc_tx_rate(struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	...
	SET_TX_DESC_TX_RATE_8192F(ptxdesc, MRateToHwRate(pattrib->rate));
...
/*setting txdesc of 10h[28:8]
10h[12:8](DATA_RTY_LOWEST_RATE)
10h[16:13](RTS_RTY_LOWEST_RATE)
10h[17](RTY_LMT_EN)
10h[23:18](RTS_DATA_RTY_LMT)
10h[28:24](RTSRate)
in case of DATA_FRAME:init_rate is useless,set 0
in case of MAGT_FRAME:lowest_rate and init_rate are useless,set 0
*/
enum FRAME_TYPE {
	RTS_CTS_FRAME= 0,
	DATA_FRAME = 1,
...
static void fill_txdesc_rate_limit_8192f(
	PADAPTER padapter,
	struct pkt_attrib *pattrib,
	u8 *ptxdesc,
	u8 frame_type,
	u8 init_rate)
{
	...
	u8 retry_count = RL_VAL_STA;
	#else
	u8 retry_count = 0x30; /*REG 42a*/
	#endif

	SET_TX_DESC_RETRY_LIMIT_ENABLE_8192F(ptxdesc, _TRUE);
	switch (frame_type) {
	case RTS_CTS_FRAME:
		SET_TX_DESC_RTS_RATE_8192F(ptxdesc, init_rate); /* RTS Rate=24M */
		SET_TX_DESC_RTS_RATE_FB_LIMIT_8192F(ptxdesc, 0xF);
		SET_TX_DESC_DATA_RETRY_LIMIT_8192F(ptxdesc, retry_count);
		break;
	case DATA_FRAME:
		SET_TX_DESC_DATA_RATE_FB_LIMIT_8192F(ptxdesc, 0x1F);
		if (pattrib->vcs_mode == NONE_VCS)  /*in func"fill_txdesc_vcs_8192f" is set*/
			SET_TX_DESC_DATA_RETRY_LIMIT_8192F(ptxdesc, retry_count);
		break;
	case MAGT_FRAME:
...
static void fill_txdesc_vcs_8192f(PADAPTER padapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	/* RTW_INFO("cvs_mode=%d\n", pattrib->vcs_mode); */
	...
	if (pattrib->vcs_mode) {
		...
		case RTS_CTS:
			SET_TX_DESC_RTS_ENABLE_8192F(ptxdesc, 1);
			SET_TX_DESC_HW_RTS_ENABLE_8192F(ptxdesc, 1);
			break;

		case CTS_TO_SELF:
			SET_TX_DESC_CTS2SELF_8192F(ptxdesc, 1);
			break;
		...
		/*setting RTS or CTS rate limit ,inital RTS Rate=24M */
		fill_txdesc_rate_limit_8192f(padapter, pattrib, ptxdesc,RTS_CTS_FRAME, _24M_RATE_);

		if (padapter->mlmeextpriv.mlmext_info.preamble_mode == PREAMBLE_SHORT)
			SET_TX_DESC_RTS_SHORT_8192F(ptxdesc, 1);

		/* Set RTS BW */
		if (pattrib->ht_en)
			SET_TX_DESC_RTS_SC_8192F(ptxdesc, SCMapping_8192F(padapter, pattrib));
	}
}

static void fill_txdesc_phy_8192f(PADAPTER padapter, struct pkt_attrib *pattrib, u8 *ptxdesc)
{
	/* RTW_INFO("bwmode=%d, ch_off=%d\n", pattrib->bwmode, pattrib->ch_offset); */

	if (pattrib->ht_en) {
		SET_TX_DESC_DATA_BW_8192F(ptxdesc, BWMapping_8192F(padapter, pattrib));
		SET_TX_DESC_DATA_SC_8192F(ptxdesc, SCMapping_8192F(padapter, pattrib));
	}
}
u8
Hal_ReadRFEType_8192F(PADAPTER Adapter, u8 *PROMContent, BOOLEAN AutoloadFail)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(Adapter);
	...
	if (!AutoloadFail) {
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			...
			if (pHalData->rfe_type > EETYPE_MAX_RFE_8192F)
				goto  fail;

		} else if (0xFF == PROMContent[EEPROM_RFE_OPTION_8192F]){
				goto  fail;
		} else {

			pHalData->rfe_type = PROMContent[EEPROM_RFE_OPTION_8192F] & 0x1f; /* 0xCA[4:0] */
		}
	} else {
		if (GetRegRFEType(Adapter) != CONFIG_RTW_RFE_TYPE) {
			pHalData->rfe_type = GetRegRFEType(Adapter);
			...
			if (pHalData->rfe_type > EETYPE_MAX_RFE_8192F)
				...
				if(0xFF == PROMContent[EEPROM_RFE_OPTION_8192F])
					goto  fail;
				else
					goto exit;
		}		
...
fail:
	...
	RTW_ERR("cmd: insmod rtl8192fx.ko rtw_RFE_type=<rfe_type>\n\n");
	return _FAIL;
}

static void rtl8192f_fill_default_txdesc(
	...
	u8 *pbuf)
{
	...
	if (pHalData->rf_type == RF_2T2R) {
		SET_TX_DESC_PATH_A_EN_8192F(pbuf, 1);
		SET_TX_DESC_PATH_B_EN_8192F(pbuf, 1);
	}
	if (pxmitframe->frame_tag == DATA_FRAMETAG) {
		u8 drv_userate = 0;

		SET_TX_DESC_MACID_8192F(pbuf, pattrib->mac_id);
		SET_TX_DESC_RATE_ID_8192F(pbuf, pattrib->raid);
		SET_TX_DESC_QUEUE_SEL_8192F(pbuf, pattrib->qsel);
		SET_TX_DESC_SEQ_8192F(pbuf, pattrib->seqnum);

		SET_TX_DESC_SEC_TYPE_8192F(pbuf, fill_txdesc_sectype(pattrib));

		if (bmcst)
			fill_txdesc_force_bmc_camid(pattrib, pbuf);

		fill_txdesc_vcs_8192f(padapter, pattrib, pbuf);
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				SET_TX_DESC_AGG_ENABLE_8192F(pbuf, 1);
				SET_TX_DESC_MAX_AGG_NUM_8192F(pbuf, 0x1F);
				SET_TX_DESC_AMPDU_DENSITY_8192F(pbuf, pattrib->ampdu_spacing);
			} else
				SET_TX_DESC_BK_8192F(pbuf, 1);

			fill_txdesc_phy_8192f(padapter, pattrib, pbuf);

			fill_txdesc_rate_limit_8192f(padapter, pattrib, pbuf,DATA_FRAME,0);

			if (pHalData->fw_ractrl == _FALSE) {
				SET_TX_DESC_USE_RATE_8192F(pbuf, 1);

				if (pHalData->INIDATA_RATE[pattrib->mac_id] & BIT(7))
					SET_TX_DESC_DATA_SHORT_8192F(pbuf, 1);

				SET_TX_DESC_TX_RATE_8192F(pbuf, pHalData->INIDATA_RATE[pattrib->mac_id] & 0x7F);
			}
			if (bmcst)
			...
			if (padapter->fix_rate != 0xFF) {
				...
				if (padapter->fix_rate & BIT(7))
					SET_TX_DESC_DATA_SHORT_8192F(pbuf, 1);
				SET_TX_DESC_TX_RATE_8192F(pbuf, padapter->fix_rate & 0x7F);
				if (!padapter->data_fb)
					SET_TX_DESC_DISABLE_FB_8192F(pbuf, 1);
			}

			if (pattrib->stbc)
				SET_TX_DESC_DATA_STBC_8192F(pbuf, 1);

#ifdef CONFIG_CMCC_TEST
			SET_TX_DESC_DATA_SHORT_8192F(pbuf, 1); /* use cck short premble */
#endif
		} else {
			...
			if((pattrib->ether_type == ETH_P_ARP) &&
				...
				SET_TX_DESC_TX_RATE_8192F(pbuf, MRateToHwRate(IEEE80211_OFDM_RATE_6MB));
			 else
#endif/*CONFIG_IP_R_MONITOR*/
				SET_TX_DESC_TX_RATE_8192F(pbuf, MRateToHwRate(pmlmeext->tx_rate));
		...
		SET_TX_DESC_USB_TXAGG_NUM_8192F(pbuf, pxmitframe->agg_num);
#endif
...
#endif
			SET_TX_DESC_CCX_8192F(pbuf, 1);
			SET_TX_DESC_SW_DEFINE_8192F(pbuf, (u8)(GET_PRIMARY_ADAPTER(padapter)->xmitpriv.seq_no));
		}
		...
		SET_TX_DESC_RATE_ID_8192F(pbuf, pattrib->raid);
		SET_TX_DESC_SEQ_8192F(pbuf, pattrib->seqnum);
		SET_TX_DESC_MBSSID_8192F(pbuf, pattrib->mbssid & 0xF);
		fill_txdesc_rate_limit_8192f(padapter, pattrib, pbuf,MAGT_FRAME,0);
		/*fixed rate for managment frame*/
		SET_TX_DESC_USE_RATE_8192F(pbuf, 1);
		SET_TX_DESC_DISABLE_FB_8192F(pbuf, 1);
		SET_TX_DESC_TX_RATE_8192F(pbuf, MRateToHwRate(pattrib->rate));
	...
	SET_TX_DESC_PKT_SIZE_8192F(pbuf, pattrib->last_txcmdsz);

	{
		...
		SET_TX_DESC_PKT_OFFSET_8192F(pbuf, pkt_offset);
		SET_TX_DESC_OFFSET_8192F(pbuf, offset);
	}

	if (bmcst)
		SET_TX_DESC_BMC_8192F(pbuf, 1);
	...
	/* 2010.06.23. Added by tynli. */
	if (!pattrib->qos_en)
		SET_TX_DESC_HWSEQ_EN_8192F(pbuf, 1);
 ...
 */
void rtl8192f_update_txdesc(struct xmit_frame *pxmitframe, u8 *pbuf)
{
	rtl8192f_fill_default_txdesc(pxmitframe, pbuf);

#if defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
	rtl8192f_cal_txdesc_chksum((struct tx_desc *)pbuf);
#endif
}
...
static void hw_var_set_monitor(PADAPTER adapter, u8 variable, u8 *val)
{
	...
	mon->rxfilter0 = rtw_read16(adapter, REG_RXFLTMAP0_8192F);
	mon->rxfilter1 = rtw_read16(adapter, REG_RXFLTMAP1_8192F);
	mon->rxfilter2 = rtw_read16(adapter, REG_RXFLTMAP2_8192F);
	rtw_write16(adapter, REG_RXFLTMAP0_8192F, 0xFFFF);
	rtw_write16(adapter, REG_RXFLTMAP1_8192F, 0xFFFF);
	rtw_write16(adapter, REG_RXFLTMAP2_8192F, 0xFFFF);
...
static void hw_var_set_opmode(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	if (isMonitor == _TRUE) {
		...
		if (backup->known_rxfilter) {
			...
			rtw_write16(padapter, REG_RXFLTMAP0_8192F, backup->rxfilter0);
			rtw_write16(padapter, REG_RXFLTMAP1_8192F, backup->rxfilter1);
			rtw_write16(padapter, REG_RXFLTMAP2_8192F, backup->rxfilter2);
		}
#endif /* CONFIG_WIFI_MONITOR */
		isMonitor = _FALSE;
	}
	...
	if (padapter->hw_port == HW_PORT1) {
		...
		if ((mode == _HW_STATE_STATION_) || (mode == _HW_STATE_NOLINK_)) {
			if (!rtw_mi_get_ap_num(padapter) && !rtw_mi_get_mesh_num(padapter)) {
...
#ifdef CONFIG_PCI_HCI
				UpdateInterruptMask8192FE(padapter, 0, 0, RT_BCN_INT_MASKS, 0);
#else /* !CONFIG_PCI_HCI */
				...
				UpdateInterruptMask8192FSdio(padapter, 0, SDIO_HIMR_BCNERLY_INT_MSK);
#endif
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
#if defined(CONFIG_SDIO_HCI)
				UpdateInterruptMask8192FSdio(padapter, 0, (SDIO_HIMR_TXBCNOK_MSK | SDIO_HIMR_TXBCNERR_MSK));
#endif
...
#ifdef CONFIG_PCI_HCI
			UpdateInterruptMask8192FE(padapter, RT_BCN_INT_MASKS, 0, 0, 0);
#else /* !CONFIG_PCI_HCI */
#ifdef CONFIG_INTERRUPT_BASED_TXBCN
#ifdef CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT
#if defined(CONFIG_SDIO_HCI)
			UpdateInterruptMask8192FSdio(padapter, SDIO_HIMR_BCNERLY_INT_MSK, 0);
#endif
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_EARLY_INT */

#ifdef CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR
#if defined(CONFIG_SDIO_HCI)
			UpdateInterruptMask8192FSdio(padapter, (SDIO_HIMR_TXBCNOK_MSK | SDIO_HIMR_TXBCNERR_MSK), 0);
#endif
#endif /* CONFIG_INTERRUPT_BASED_TXBCN_BCN_OK_ERR */
			...
			rtw_write16(padapter, REG_TSFTR_SYN_OFFSET, 0x640);
			...
			/* SW_BCN_SEL - Port1 */
			/* rtw_write8(Adapter, REG_DWBCN1_CTRL_8192F+2, rtw_read8(Adapter, REG_DWBCN1_CTRL_8192F+2)|BIT4); */
			rtw_hal_set_hwreg(padapter, HW_VAR_DL_BCN_SEL, NULL);

			/* select BCN on port 1 */
			rtw_write8(padapter, REG_CCK_CHECK_8192F,
				(rtw_read8(padapter, REG_CCK_CHECK_8192F) | BIT_BCN_PORT_SEL));
...
#define	GET_8192F_C2H_TX_RPT_LIFE_TIME_OVER(_Header)	LE_BITS_TO_1BYTE((_Header + 0), 6, 1)
#define	GET_8192F_C2H_TX_RPT_RETRY_OVER(_Header)	LE_BITS_TO_1BYTE((_Header + 0), 7, 1)
...
#ifdef CONFIG_XMIT_ACK
	if (GET_8192F_C2H_TX_RPT_RETRY_OVER(pdata) | GET_8192F_C2H_TX_RPT_LIFE_TIME_OVER(pdata))
...
)
{
	...
	TxOK = GET_8192F_C2H_TC2H_APREQ_TXRPT_TXOK1(CmdBuf);
	TxFail = GET_8192F_C2H_TC2H_APREQ_TXRPT_TXFAIL1(CmdBuf);
	IniRate = GET_8192F_C2H_TC2H_APREQ_TXRPT_INIRATE1(CmdBuf);
...
)
{
	...
	psta->sta_stats.tx_retry_cnt = GET_8192F_C2H_SPC_STAT_TYPEB_RETRY1(CmdBuf);
	rtw_sctx_done(&pstapriv->gotc2h);
}

s32 c2h_handler_8192f(_adapter *adapter, u8 id, u8 seq, u8 plen, u8 *payload)
{
	s32 ret = _SUCCESS;

	switch (id) {
	case C2H_CCX_TX_RPT:
		CCX_FwC2HTxRpt_8192f(adapter, payload, plen);
		break;
	case C2H_AP_REQ_TXRPT:
		C2HTxRPTHandler_8192F(adapter, payload, plen);
		break;
	case C2H_SPC_STAT:
		C2HSPC_STAT_8192F(adapter, payload, plen);
		break;
	default:
		ret = _FAIL;
		break;
...
u8 SetHwReg8192F(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_AMPDU_FACTOR: {
		...
		rtw_write32(padapter, REG_AMPDU_MAX_LENGTH_8192F, AMPDULen);
	}
		break;
	...
	case HW_VAR_H2C_FW_PWRMODE: {
		...
		/* } */
		rtl8192f_set_FwPwrMode_cmd(padapter, psmode);
	}
		break;
	case HW_VAR_H2C_PS_TUNE_PARAM:
		rtl8192f_set_FwPsTuneParam_cmd(padapter);
		break;

	case HW_VAR_H2C_FW_JOINBSSRPT:
		rtl8192f_set_FwJoinBssRpt_cmd(padapter, *val);
#ifdef CONFIG_LPS_POFF
		rtl8192f_lps_poff_h2c_ctrl(padapter, *val);
#endif
		break;
	...
	case HW_VAR_DL_RSVD_PAGE:
		...
		if (check_fwstate(&padapter->mlmepriv, WIFI_AP_STATE) == _TRUE)
			rtl8192f_download_BTCoex_AP_mode_rsvd_page(padapter);
		else
#endif /* CONFIG_BT_COEXIST */
		{
			rtl8192f_download_rsvd_page(padapter, RT_MEDIA_CONNECT);
		}
		break;

#ifdef CONFIG_P2P
	case HW_VAR_H2C_FW_P2P_PS_OFFLOAD:
		rtl8192f_set_p2p_ps_offload_cmd(padapter, *val);
		break;
#endif /* CONFIG_P2P */
#ifdef CONFIG_LPS_POFF
	case HW_VAR_LPS_POFF_INIT:
		rtl8192f_lps_poff_init(padapter);
		break;
	case HW_VAR_LPS_POFF_DEINIT:
		rtl8192f_lps_poff_deinit(padapter);
		break;
	case HW_VAR_LPS_POFF_SET_MODE:
		rtl8192f_lps_poff_set_ps_mode(padapter, *val);
		break;
...
#define RXDMA_IDLE			BIT(17)
	...
	case HW_VAR_NAV_UPPER: {
		...
		if (usNavUpper > HAL_NAV_UPPER_UNIT_8192F * 0xFF) {
			break;
		}

		/* The value of ((usNavUpper + HAL_NAV_UPPER_UNIT_8192F - 1) / HAL_NAV_UPPER_UNIT_8192F) */
		/* is getting the upper integer. */
		usNavUpper = (usNavUpper + HAL_NAV_UPPER_UNIT_8192F - 1) / HAL_NAV_UPPER_UNIT_8192F;
	...
	case HW_VAR_WOW_OUTPUT_GPIO: {
		struct pwrctrl_priv *pwrctrlpriv = adapter_to_pwrctl(padapter);
		u8 index = pwrctrlpriv->wowlan_gpio_index;
		u8 output_value = *((u8 *)val);

		if (index <= 31) {	/* GPIOA_0 ~ GPIOA_31 */
			/* config GPIO Input/Output mode ( 0: input, 1: output)*/
			rtw_write32(padapter, REG_SW_GPIO_A_OEN,
					rtw_read32(padapter, REG_SW_GPIO_A_OEN) | BIT(index));

			/* config high/low state for GPIO output mode ( 0: low, 1: high)*/
			if (output_value) {
				rtw_write32(padapter, REG_SW_GPIO_A_OUT,
					rtw_read32(padapter, REG_SW_GPIO_A_OUT) | BIT(index));
			} else {
				rtw_write32(padapter, REG_SW_GPIO_A_OUT,
					rtw_read32(padapter, REG_SW_GPIO_A_OUT) & ~BIT(index));
			}
		} else if (index >= 32 && index <= 39) {	/* GPIOB_0 ~ GPIOB_7 */

			/* shift GPIO index to category GPIOB index, if the controlled index is over 32 */
			index = index % 32;
		
			/* config GPIO Input/Output mode ( 0: input, 1: output)*/
			rtw_write8(padapter, REG_SW_GPIO_B_OEN,
					rtw_read8(padapter, REG_SW_GPIO_B_OEN) | BIT(index));

			/* config high/low state for GPIO output mode ( 0: low, 1: high)*/
			if (output_value) {
				rtw_write8(padapter, REG_SW_GPIO_B_OUT,
					rtw_read8(padapter, REG_SW_GPIO_B_OUT) | BIT(index));
			} else {
				rtw_write8(padapter, REG_SW_GPIO_B_OUT,
					rtw_read8(padapter, REG_SW_GPIO_B_OUT) & ~BIT(index));
			}
		} else {
			RTW_INFO("%s: invalid GPIO%d=%d\n", __func__, index, output_value);
		}
	}
		break;

	case HW_VAR_WOW_INPUT_GPIO: {
		u8 index = *((u8 *)val);
...
struct qinfo_8192f {
	u8 head;/*400[7:0]*/
	u8 pkt_num;/*400[15:8];length change*/
	u8 tail;/*400[23:16]*/
	u8 ac:2;/*404[9:8]*/
	u8 macid:7;/*404[6:0]*/
};

struct bcn_qinfo_8192f {
	u16 head:8;
	u16 pkt_num:8;
};

void dump_acqinfo_8192f(void *sel, struct qinfo_8192f *info, u8 tag)
{
	/* if (info->pkt_num) */
	RTW_PRINT_SEL(sel, "Q%3u head:0x%0x, tail:0x%0x, pkt_num:%u, macid:%u, ac:%u\n"
		,tag , info->head, info->tail,
		info->pkt_num, info->macid, info->ac);
}

void dump_qinfo_8192f(void *sel, struct qinfo_8192f *info, const char *tag)
{
	/* if (info->pkt_num) */
	if(*tag != 'B')
		RTW_PRINT_SEL(sel, "%4s head:0x%02x, tail:0x%02x, pkt_num:%u, macid:%u, ac:%u\n"
			, tag ? tag : "", info->head, info->tail, info->pkt_num, info->macid, info->ac);
	else
		RTW_PRINT_SEL(sel, "%4s head:0x%02x, tail:0x%02x, pkt_num:%u, macid:%u\n"
			, tag ? tag : "", info->head, info->tail, info->pkt_num, info->macid);
}

void dump_mac_qinfo_8192f(void *sel, _adapter *adapter)
{
	struct qinfo_8192f q_info[QUEUE_TOTAL_NUM];
	struct qinfo_8192f tmp_q_info;
	u8 q_index, value;
	for(q_index = 0; q_index < QUEUE_TOTAL_NUM; q_index++) {
		struct qinfo_8192f *tmp_q_info = &q_info[q_index];
		value = rtw_read8(adapter, REG_QUEUELIST_INFO2_8192F);

		RTW_DBG("%s: %0x = 0x%x \n",__func__,REG_QUEUELIST_INFO2_8192F,value );

		value &=0xe0;/*Index set in 0x414[4:0])*/

		RTW_INFO("%s: %0x index = %0x \n",__func__,REG_QUEUELIST_INFO2_8192F ,value |q_index);

		
		rtw_write8(adapter, REG_QUEUELIST_INFO2_8192F, value |q_index);/*set index*/
		RTW_DBG("%s: %0x = %0x \n",__func__,REG_QUEUELIST_INFO0_8192F,rtw_read32(adapter, REG_QUEUELIST_INFO0_8192F) );
		RTW_DBG("%s: %0x = %0x \n",__func__,REG_QUEUELIST_INFO1_8192F,rtw_read32(adapter, REG_QUEUELIST_INFO1_8192F) );
		tmp_q_info->head = rtw_read8(adapter, REG_QUEUELIST_INFO0_8192F);
		tmp_q_info->pkt_num = rtw_read8(adapter, REG_QUEUELIST_INFO0_8192F+1);
		tmp_q_info->tail = rtw_read8(adapter, REG_QUEUELIST_INFO0_8192F+2);
		tmp_q_info->macid = ((rtw_read8(adapter, REG_QUEUELIST_INFO1_8192F)) & 0x7f);
		if(q_index != QUEUE_INDEX_BCNQ)
			tmp_q_info->ac =  ((rtw_read8(adapter, REG_QUEUELIST_INFO1_8192F+1)) & 0x03);
			/*BCNQ no AC info*/
		if(q_index < QUEUE_ACQ_NUM)
			dump_acqinfo_8192f(sel, tmp_q_info, q_index);
		else{
			switch(q_index){
				case QUEUE_INDEX_MGQ:
					dump_qinfo_8192f(sel, tmp_q_info, "MG ");
					break;
				case QUEUE_INDEX_HIQ:
					dump_qinfo_8192f(sel, tmp_q_info, "HI ");
					break;
				case QUEUE_INDEX_BCNQ:
					dump_qinfo_8192f(sel, tmp_q_info, "BCN ");
					break;
				case QUEUE_INDEX_CMDQ:
					dump_qinfo_8192f(sel, tmp_q_info, "CMD ");
					break;
			}
		}
...
static void dump_mac_txfifo_8192f(void *sel, _adapter *adapter)
{
	u32 rqpn, rqpn_npq, rqpn_ex;
	u32 hpq, lpq, npq, epq, pubq, exq1, exq2;

	rqpn = rtw_read32(adapter, REG_FIFOPAGE);
	rqpn_npq = rtw_read32(adapter, REG_RQPN_NPQ_8192F);
	rqpn_ex = rtw_read32(adapter, REG_RQPN_EXQ1_EXQ2);
	...
	exq1 = ((rqpn_ex & 0xFF00)>>8);
	exq2 = ((rqpn_ex & 0xFF0000)>>16);

	RTW_PRINT_SEL(sel, "Tx: available page num: ");
	if ((hpq == 0xEA) && (hpq == lpq) && (hpq == pubq))
		RTW_PRINT_SEL(sel, "N/A (reg val = 0xea)\n");
	else
		RTW_PRINT_SEL(sel, "HPQ: %d, LPQ: %d, NPQ: %d, EPQ: %d, PUBQ: %d, EXQ1: %d, EXQ2: %d \n"
			, hpq, lpq, npq, epq, pubq, exq1, exq2);
}
#endif

void rtl8192f_read_wmmedca_reg(PADAPTER adapter, u16 *vo_params, u16 *vi_params, u16 *be_params, u16 *bk_params)
{
	u8 vo_reg_params[4];
	u8 vi_reg_params[4];
	u8 be_reg_params[4];
	...
	GetHwReg8192F(adapter, HW_VAR_AC_PARAM_VO, vo_reg_params);
	GetHwReg8192F(adapter, HW_VAR_AC_PARAM_VI, vi_reg_params);
	GetHwReg8192F(adapter, HW_VAR_AC_PARAM_BE, be_reg_params);
	GetHwReg8192F(adapter, HW_VAR_AC_PARAM_BK, bk_reg_params);
...
void GetHwReg8192F(PADAPTER padapter, u8 variable, u8 *val)
{
	...
	case HW_VAR_CHK_HI_QUEUE_EMPTY:
		val32 = rtw_read32(padapter, REG_TXPKT_EMPTY_8192F);
		*val = (val32 & BIT(17)) ? _TRUE : _FALSE;
		break;
	case HW_VAR_CHK_MGQ_CPU_EMPTY:
		val32 = rtw_read32(padapter, REG_TXPKT_EMPTY_8192F);
		*val = (val32 & BIT(19)) ? _TRUE : _FALSE;
		break;
#ifdef CONFIG_WOWLAN
	...
	case HW_VAR_DUMP_MAC_QUEUE_INFO:
		dump_mac_qinfo_8192f(val, padapter);
		break;
	case HW_VAR_DUMP_MAC_TXFIFO:
		dump_mac_txfifo_8192f(val, padapter);
		break;
#endif
	default:
...
void hal_ra_info_dump(_adapter *padapter , void *sel)
{
	...
	for (i = 0; i < macid_ctl->num; i++) {
		...
		if (rtw_macid_is_used(macid_ctl, i) && !rtw_macid_is_bmc(macid_ctl, i)) {
			...
			rtw_write32(padapter, REG_HMEBOX_DBG_2_8192F, cmd);
			rtw_msleep_os(10);
...
u8 GetHalDefVar8192F(PADAPTER padapter, HAL_DEF_VARIABLE variable, void *pval)
{
	...
	case HAL_DEF_TX_PAGE_BOUNDARY:
		if (!padapter->registrypriv.wifi_spec)
			*(u8 *)pval = TX_PAGE_BOUNDARY_8192F;
		else
			*(u8 *)pval = WMM_NORMAL_TX_PAGE_BOUNDARY_8192F;
		break;
	case HAL_DEF_TX_PAGE_SIZE:
		*((u32 *)pval) = PAGE_SIZE_256;
		break;
	case HAL_DEF_TX_BUFFER_LAST_ENTRY:
		*(u8 *)pval = LAST_ENTRY_OF_TX_PKT_BUFFER_8192F;
		break;
	case HAL_DEF_RX_DMA_SZ_WOW:
		*(u32 *)pval = RX_DMA_SIZE_8192F - RESV_FMWF;
		break;
	case HAL_DEF_RX_DMA_SZ:
		*(u32 *)pval = RX_DMA_BOUNDARY_8192F + 1;
		break;
...
void rtl8192f_start_thread(_adapter *padapter)
{
	...
	if (xmitpriv->SdioXmitThread == NULL) {
		...
		xmitpriv->SdioXmitThread = kthread_run(rtl8192fs_xmit_thread, padapter, "RTWHALXT");
		if (IS_ERR(xmitpriv->SdioXmitThread)) {
			RTW_ERR("%s: start rtl8192fs_xmit_thread FAIL!!\n", __func__);
...
void rtl819fs_init_checkbthang_workqueue(_adapter *adapter)
{
...
#define PRECNT_TH		0x230 /*7us */

/* pre-tx count-down mechanism */
void rtl8192f_pretx_cd_config(_adapter *adapter)
{
	u8 burst_mode;
	u16 pre_cnt = PRECNT_TH | BIT_EN_PRECNT_8192F;

	/*Enable AMPDU PRE-TX, Reg0x4BC[6] = 1*/
	burst_mode = rtw_read8(adapter, REG_AMPDU_BURST_MODE_8192F);
	if (!(burst_mode & BIT_PRE_TX_CMD_8192F)) {
		burst_mode |= BIT_PRE_TX_CMD_8192F;
		rtw_write8(adapter, REG_AMPDU_BURST_MODE_8192F, burst_mode);
	}

	/*Reg0x4E5[11] = 1, Reg0x4E5[10:0] = 0x1E4 */
	rtw_write8(adapter, REG_PRECNT_CTRL_8192F, (u8)(pre_cnt & 0xFF));
	rtw_write8(adapter, (REG_PRECNT_CTRL_8192F + 1), (u8)(pre_cnt >> 8));
...
{
	pHalFunc->dm_init = &rtl8192f_init_dm_priv;
	pHalFunc->dm_deinit = &rtl8192f_deinit_dm_priv;
	pHalFunc->read_chip_version = read_chip_version_8192f;
	pHalFunc->set_chnl_bw_handler = &PHY_SetSwChnlBWMode8192F;
	pHalFunc->set_tx_power_level_handler = &PHY_SetTxPowerLevel8192F;
	pHalFunc->set_tx_power_index_handler = &PHY_SetTxPowerIndex_8192F;
	pHalFunc->get_tx_power_index_handler = hal_com_get_txpwr_idx;
	pHalFunc->hal_dm_watchdog = &rtl8192f_HalDmWatchDog;

	pHalFunc->SetBeaconRelatedRegistersHandler = &rtl8192f_SetBeaconRelatedRegisters;
	pHalFunc->run_thread = &rtl8192f_start_thread;
	pHalFunc->cancel_thread = &rtl8192f_stop_thread;
	pHalFunc->read_bbreg = &PHY_QueryBBReg_8192F;
	pHalFunc->write_bbreg = &PHY_SetBBReg_8192F;
	pHalFunc->read_rfreg = &PHY_QueryRFReg_8192F;
	pHalFunc->write_rfreg = &PHY_SetRFReg_8192F;
	pHalFunc->read_wmmedca_reg = &rtl8192f_read_wmmedca_reg;
	...
	pHalFunc->sreset_xmit_status_check = &rtl8192f_sreset_xmit_status_check;
	pHalFunc->sreset_linked_status_check  = &rtl8192f_sreset_linked_status_check;
...
#endif
	pHalFunc->hal_notch_filter = &hal_notch_filter_8192f;
	pHalFunc->c2h_handler = c2h_handler_8192f;
	pHalFunc->fill_h2c_cmd = &FillH2CCmd8192F;
	pHalFunc->fill_fake_txdesc = &rtl8192f_fill_fake_txdesc;
	pHalFunc->reqtxrpt = rtl8192f_req_txrpt_cmd;
	pHalFunc->fw_dl = &rtl8192f_FirmwareDownload;
	pHalFunc->hal_get_tx_buff_rsvd_page_num = &GetTxBufferRsvdPageNum8192F;
	#if defined (CONFIG_CONCURRENT_MODE)  && defined (CONFIG_TSF_SYNC)
	pHalFunc->tsf_sync = &rtl8192f_sync_tsfr;
	#endif
}
