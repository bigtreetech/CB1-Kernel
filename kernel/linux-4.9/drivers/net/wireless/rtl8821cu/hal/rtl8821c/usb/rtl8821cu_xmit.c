...
#include "rtl8821cu.h"			/* OFFSET_SZ MAX_TX_AGG_PACKET_NUMBER_8821C */
...
static void update_txdesc_h2c_pkt(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz)
{
	...
	SET_TX_DESC_QSEL_8821C(ptxdesc, HALMAC_TXDESC_QSEL_H2C_CMD);
...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz, u8 bagg_pkt)
{
	...
	SET_TX_DESC_OFFSET_8821C(ptxdesc, offset);
	...
	if (pxmitframe->pkt_offset > 0)
		SET_TX_DESC_PKT_OFFSET_8821C(ptxdesc, pxmitframe->pkt_offset);
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		if (pxmitframe->agg_num > 1) {
			...
			SET_TX_DESC_DMA_TXAGG_NUM_8821C(ptxdesc, pxmitframe->agg_num);
		}
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				...
				SET_TX_DESC_MAX_AGG_NUM_8821C(ptxdesc, max_agg_num);
				/* Set A-MPDU aggregation */
				SET_TX_DESC_AMPDU_DENSITY_8821C(ptxdesc, pattrib->ampdu_spacing);
			} else
#endif/*CONFIG_80211N_HT*/
			...
			if (pHalData->current_band_type == BAND_ON_5G)
				SET_TX_DESC_DATA_RTY_LOWEST_RATE_8821C(ptxdesc, 4);
			else
				SET_TX_DESC_DATA_RTY_LOWEST_RATE_8821C(ptxdesc, 0);
			...
			if (pHalData->fw_ractrl == _FALSE) {
				...
				SET_TX_DESC_DATARATE_8821C(ptxdesc, (pHalData->INIDATA_RATE[pattrib->mac_id] & 0x7F));
			}
			if (bmcst) {
				DriverFixedRate = 0x01;
			...
			if (padapter->fix_rate != 0xFF) {
				...
				SET_TX_DESC_DATARATE_8821C(ptxdesc, (padapter->fix_rate & 0x7F));
		...
		if ((pattrib->subtype == WIFI_NDPA) ||
			...
			SET_TX_DESC_NAVUSEHDR_8821C(ptxdesc, 1);

			SET_TX_DESC_DATA_BW_8821C(ptxdesc, rtl8821c_bw_mapping(padapter, pattrib));
			SET_TX_DESC_SIGNALING_TA_PKT_SC_8821C(ptxdesc,
					rtl8821c_sc_mapping(padapter, pattrib));

			SET_TX_DESC_RTY_LMT_EN_8821C(ptxdesc, 1);
			SET_TX_DESC_RTS_DATA_RTY_LMT_8821C(ptxdesc, 5);
			SET_TX_DESC_DISRTSFB_8821C(ptxdesc, 1);
			SET_TX_DESC_NDPA_8821C(ptxdesc, 1);
		} else {
			SET_TX_DESC_RTY_LMT_EN_8821C(ptxdesc, 1);
...
static s32 rtl8821cu_xmitframe_complete(PADAPTER padapter, struct xmit_priv *pxmitpriv, struct xmit_buf *pxmitbuf)
{
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);
	struct dvobj_priv *pdvobj = adapter_to_dvobj(padapter);
...
#endif
		if (MAX_TX_AGG_PACKET_NUMBER_8821C == pfirstframe->agg_num)
			break;
...
static void rtl8821cu_xmit_tasklet(void *priv)
{
	...
	while (1) {
		...
		ret = rtl8821cu_xmitframe_complete(padapter, pxmitpriv, NULL);
...
s32	rtl8821cu_init_xmit_priv(PADAPTER padapter)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	HAL_DATA_TYPE *pHalData = GET_HAL_DATA(padapter);
...
void	rtl8821cu_free_xmit_priv(PADAPTER padapter)
{
}
...
s32 rtl8821cu_mgnt_xmit(PADAPTER padapter, struct xmit_frame *pmgntframe)
{
 ...
 */
s32 rtl8821cu_hal_xmit(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	return pre_xmitframe(padapter, pxmitframe);
}

s32 rtl8821cu_hal_xmitframe_enqueue(PADAPTER padapter, struct xmit_frame *pxmitframe)
{
	struct xmit_priv *pxmitpriv = &padapter->xmitpriv;
	s32 err;
...
static void rtl8821cu_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8821cu_hostap_mgnt_xmit_entry(PADAPTER padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
