				...
				struct wsm_buf *buf, int interface_link_id)
{
	...
	DBG_ARRY_ADD(dbg_txconfirm, count);
					...
					struct wsm_buf *buf)
{
...
#if FW_RECORD_READ
	hw_priv->wsm_caps.firmwareConfig[3] = WSM_GET32(buf);
#endif
	...
	if (hw_priv->hw_revision == XR819_HW_REV1) {
		WSM_GET(buf, hw_priv->wsm_caps.efuse_mac,
				sizeof(hw_priv->wsm_caps.efuse_mac));
		hw_priv->wsm_caps.efuse_flag = WSM_GET8(buf);
		wsm_printk(XRADIO_DBG_ALWY,
			"Efuse Flag=0x%02x, MAC Addr=%02x:%02x:%02x:%02x:%02x:%02x\n",
			hw_priv->wsm_caps.efuse_flag,
			hw_priv->wsm_caps.efuse_mac[0], hw_priv->wsm_caps.efuse_mac[1],
			hw_priv->wsm_caps.efuse_mac[2], hw_priv->wsm_caps.efuse_mac[3],
			hw_priv->wsm_caps.efuse_mac[4], hw_priv->wsm_caps.efuse_mac[5]);
	}
	...
	*/
	...
	if (measure_cmpl.Status == WSM_STATUS_SUCCESS) {
		...
		case ChannelLoadMeasurement:
...
#endif
#ifdef SUPPORT_ACS
			sband = hw_priv->hw->wiphy->bands[NL80211_BAND_2GHZ];
			if (!sband)
				chan = NULL;
			else {
				for (i = 0; i < sband->n_channels; i++) {
					chan = &sband->channels[i];
					if (chanload_res->ChannelNum == chan->hw_value)
						break;
					else
						chan = NULL;
				}
			...
			hw_priv->survey.channel = chan;
			hw_priv->survey.channel_time = (u64)chanload_res->MeasurementDuration;
			busytime = (u64)cca_chanload;
			busytime = busytime * (hw_priv->survey.channel_time);
			busytime = busytime >> 8;
			hw_priv->survey.channel_time_busy = busytime;
			hw_priv->survey.noise = (s8)chanload_res->NoiseLeveldBm;
			hw_priv->survey.filled = SURVEY_INFO_CHANNEL_TIME |
								SURVEY_INFO_CHANNEL_TIME_BUSY |
								SURVEY_INFO_NOISE_DBM;

			/* fill channel survey debug messages */
			if (chan->hw_value <= 14) {
				mutex_lock(&hw_priv->ms_mutex);
				memcpy(&(hw_priv->debug_survey[chan->hw_value - 1]),
					&(hw_priv->survey), sizeof(struct survey_info));
				mutex_unlock(&hw_priv->ms_mutex);
			}

			atomic_set(&hw_priv->ms_status, 1);
			wake_up(&hw_priv->ms_wq);
#endif
		 ...
		 void *arg, u16 cmd, long tmo, int if_id)
{
	...
	if (unlikely(ret == 0)) {
		u16 raceCheck;
		wsm_printk(XRADIO_DBG_ERROR,
			   "***CMD timeout!>>> 0x%.4X (%zu), cmd_ptr=%p, buf_use=%d," \
			   "bh_state=%d\n", cmd, buf_len, hw_priv->wsm_cmd.ptr,
			   hw_priv->hw_bufs_used, hw_priv->bh_error);
	...
	if (hw_priv->bh_error) {
		...
		while (xradio_query_txpkt_timeout(hw_priv, XRWL_ALL_IFS,
			...
			if (timeout < 0 || !num) {
				...
				wsm_printk(XRADIO_DBG_ERROR, "%s:hw_bufs_used=%d, num=%d\n",
					   __func__, hw_priv->hw_bufs_used, num);
				hw_priv->bh_error = 1;
#ifdef BH_USE_SEMAPHORE
				up(&hw_priv->bh_sem);
#else
...
bool wsm_vif_flush_tx(struct xradio_vif *priv)
{
	...
	if (hw_priv->bh_error) {
		...
		while (xradio_query_txpkt_timeout(hw_priv, if_id,
		       0xffffffff, &timeout)) {
			if (timeout < 0 || !hw_priv->hw_bufs_used_vif[if_id]) {
				/* Hmmm... Not good. Frame had stuck in firmware. */
				wsm_printk(XRADIO_DBG_ERROR,
					   "%s: if_id=%d, hw_bufs_used_vif=%d, time=%ld\n",
					   __func__, if_id,
					   hw_priv->hw_bufs_used_vif[priv->if_id], jiffies - time);
				hw_priv->bh_error = 1;
	#ifdef BH_USE_SEMAPHORE
				up(&hw_priv->bh_sem);
	#else
			...
			ret = wait_event_timeout(hw_priv->bh_evt_wq, !hw_priv->hw_bufs_used_vif[if_id], timeout);
			if (ret > 0) {
				return true;
			}
		}
						...
						struct wsm_buf       *buf)
{
	...
	if (dbg_id == 5) {
		...
		int i = 0;
		wsm_printk(XRADIO_DBG_ALWY,
			"[FW-DEBUG] DbgId = %d, Len = %d\n", dbg_id, dbg_buf_len);
		while (dbg_buf_len >= 4) {
			wsm_printk(XRADIO_DBG_ALWY,
				"[FW-DEBUG] Data%d = 0x%08x\n", i, WSM_GET32(buf));
			dbg_buf_len -= 4;
			i++;
		}
	}
...
int wsm_hwt_tx_confirm(struct xradio_common *hw_priv, struct wsm_buf *buf)
{
	...
	if (*(buf->data+7) & 0x01) { /*last packet*/
		...
		wsm_printk(XRADIO_DBG_ALWY,
		    "%s, HIF TX: time=%dms, throughput=%d.%dMbps, SW time=%dus\n",
