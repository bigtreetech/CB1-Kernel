...
void mmc_set_clock(struct mmc_host *host, unsigned int hz)
{
    ...
    char *p = mmc_hostname(host);
    if( strstr( p,"mmc1") !=NULL)
    {
       if(!wifi_on_flag) //kandy
       {
          
         // printk("==> wifi is not opened,mmc_set_clock return\n");
          // return;
       }        
    }
	...
	host->ios.clock = hz;
	mmc_set_ios(host);
  // printk("==>%s: host->f_max = %d , max clk = %d,\n",__func__,host->f_max,hz);
}
...
u32 mmc_select_voltage(struct mmc_host *host, u32 ocr)
{
  ...
  //  return 0x00200000; //kandy
	...
	//printk("==> mmc_select_voltage return %08x\n",ocr);
...
int __mmc_set_signal_voltage(struct mmc_host *host, int signal_voltage)
{
    ...
    //printk("==> %s: volt = %08x\n",__func__,host->ios.signal_voltage);
...
void mmc_power_up(struct mmc_host *host, u32 ocr)
{
	...
	if (__mmc_set_signal_voltage(host, MMC_SIGNAL_VOLTAGE_330) == 0)
    {
       ...
       // printk("==> %s: mmc signal volt = 3.3V\n",__func__);
    }
	else if (__mmc_set_signal_voltage(host, MMC_SIGNAL_VOLTAGE_180) == 0)
    {
		dev_dbg(mmc_dev(host), "Initial signal voltage of 1.8v\n");
        //printk("==> %s: mmc signal volt = 1.8V\n",__func__);
    }
	else if (__mmc_set_signal_voltage(host, MMC_SIGNAL_VOLTAGE_120) == 0)
    {
		dev_dbg(mmc_dev(host), "Initial signal voltage of 1.2v\n");
       // printk("==> %s: mmc signal volt = 1.2V\n",__func__);
    }
...
void mmc_rescan(struct work_struct *work)
{
   // printk("==> mmc_rescan enter\n");    
    ...
    {
        printk("==> mmc_rescan return : rescan_disable\n");
		return;
    }
    ...
    {
        printk("==> mmc_rescan return: mmc_card_is_removable\n");
		return;
    }
	...
	if (host->bus_ops != NULL) {
		mmc_bus_put(host);
       // printk("==> mmc_rescan out: mmc_bus_put\n");
		goto out;
	}
	...
	if (mmc_card_is_removable(host) && host->ops->get_cd &&
		...
		mmc_power_off(host);
		mmc_release_host(host);
        printk("==> mmc_rescan out: mmc_power_off\n");
		goto out;
	}
