...
ret1=i2c_sendbyte(i2c_device_addr<<1);
...
pr_err("i2c_write error ret1=%d,ret2=%d,ret3=%d\n",ret1,ret2,ret3);
...
static u8 i2c_read_reg(u8 reg){
    ...
    ret3=i2c_sendbyte((i2c_device_addr<<1)+1);
    ...
    if(ret1||ret2||ret3){
        pr_err("i2c_write error ret1=%d,ret2=%d,ret3=%d,ret4=%d\n",ret1,ret2,ret3,ret4);
        return -ENXIO;
    }
...
static void i2c_read_reg_block(u8 reg,u8 nbytes,u8 *data){
 ...
 */
#define NS2009_PEN_UP_Z1_ERR	1//80
...
static int ns2009_ts_report(struct ns2009_data *data)
{
   ...
   // printk("==> ns2009 report: z1 = %d\n",z1);
	...
	if (z1 >= NS2009_PEN_UP_Z1_ERR) {
		...
		printk("%s[%d] z1:%d, x:%d, y:%d\n", __func__, __LINE__, z1, x, y);
