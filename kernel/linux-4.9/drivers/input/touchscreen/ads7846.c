...
{
    printk("==> __ads7846_enable\n");
	ads7846_restart(ts);
}

static void ads7846_disable(struct ads7846 *ts)
{
    printk("==> ads7846_disable\n");
	mutex_lock(&ts->lock);
...
static int ads7846_read12_ser(struct device *dev, unsigned command)
{
    ...
    printk("==> ads7846_read12_ser enter\n");
...
static int get_pendown_state(struct ads7846 *ts)
{
    ...
    int ret = !gpio_get_value(ts->gpio_pendown);
    
    printk("==> get_pendown_state: %d\n",ret);
...
static irqreturn_t ads7846_irq(int irq, void *handle)
{
	...
	while (!ts->stopped && get_pendown_state(ts)) {
        ...
        printk("==> ads7846_irq:measurement \n");
...
static const struct ads7846_platform_data *ads7846_probe_dt(struct device *dev)
{
	...
	of_property_read_u16(node, "vref-delay-usecs",
			     &pdata->vref_delay_usecs);
	of_property_read_u16(node, "vref-mv", &pdata->vref_mv);
	pdata->keep_vref_on = of_property_read_bool(node, "keep-vref-on");

	pdata->swap_xy = of_property_read_bool(node, "swap-xy");

	of_property_read_u16(node, "settle-delay-usec",
			     &pdata->settle_delay_usecs);
	of_property_read_u16(node, "penirq-recheck-delay-usecs",
...
static int ads7846_probe(struct spi_device *spi)
{
    ...
    ts_irq = gpio_to_irq(GPIO_TS_IRQ);
	...
	if (spi->max_speed_hz > (125000 * SAMPLE_BITS)) {
        ...
        printk("------------------- ===> xpt2046 max_speed_hz error\n");
		return -EINVAL;
	}
	
	printk("==> ts spi->max_speed_hz = %d \n",spi->max_speed_hz);
	...
	if (!pdata) {
		...
		if (IS_ERR(pdata)) {
			err = PTR_ERR(pdata);
            printk("==> ads7846 dev_get_platdata failed\n");
			goto err_free_mem;
		}
	}
	...
	if (pdata->filter != NULL) {
		if (pdata->filter_init != NULL) {
			...
			if (err < 0)
            {
                printk("==> ads7846 filter_init failed\n");
				goto err_free_mem;
            }
		}
	...
	if (err)
    {
        printk("==> ads7846_setup_pendown failed\n");
...
*/
	...
	err = request_threaded_irq(ts_irq, ads7846_hard_irq, ads7846_irq,
				   irq_flags, spi->dev.driver->name, ts);
	if (err && !pdata->irq_flags) {
		dev_info(&spi->dev,
			"trying pin change workaround on irq %d\n", ts_irq);
		irq_flags |= IRQF_TRIGGER_RISING;
		err = request_threaded_irq(ts_irq,
	...
	if (err)
    {
        printk("==> ads784x_hwmon_register failed\n");
		goto err_free_irq;
    }
	...
	if (err)
    {
        printk("==> ads784x sysfs_create_group failed\n");
	...
	if (err)
    {
        printk("==> ads784x input_register_device failed\n");
