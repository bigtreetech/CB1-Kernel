                           ...
                           ulong sector, struct image_header *header)
{
    ...
    image_size_sectors = (spl_image->size + mmc->read_bl_len - 1) / mmc->read_bl_len;

    /* Read the header too to avoid extra memcpy */
    count = blk_dread(mmc_get_blk_desc(mmc), sector + image_offset_sectors,
    ...
    if (image_offset)
        memmove((void *)(ulong)spl_image->load_addr,
                (void *)(ulong)spl_image->load_addr + image_offset, spl_image->size);
...
static __maybe_unused int mmc_load_image_raw_sector(struct spl_image_info *spl_image,
...
static int spl_mmc_find_device(struct mmc **mmcp, u32 boot_device)
{
    ...
    {
#ifdef CONFIG_SPL_LIBCOMMON_SUPPORT
        printf("spl: could not initialize mmc. error_num: %d\n", err);
#endif
        return err;
    }
    *mmcp = find_mmc_device(mmc_dev);
    ...
    {
#ifdef CONFIG_SPL_LIBCOMMON_SUPPORT
        printf("Err-spl: could not find mmc device %d. error_num: %d\n", mmc_dev, err);
#endif
        return err;
    }
...
static int mmc_load_image_raw_os(struct spl_image_info *spl_image, struct mmc *mmc)
{
    ...
    ret = mmc_load_image_raw_sector(spl_image, mmc, CONFIG_SYS_MMCSD_RAW_MODE_KERNEL_SECTOR);
    if (ret)
        return ret;
...
static int spl_mmc_do_fs_boot(struct spl_image_info *spl_image, struct mmc *mmc, const char *filename)
{
    int err = -ENOSYS;
    ...
    if (!spl_start_uboot())
    {
        err = spl_load_image_fat_os(spl_image, mmc_get_blk_desc(mmc), CONFIG_SYS_MMCSD_FS_BOOT_PARTITION);
        if (!err)
            return err;
    }
#ifdef CONFIG_SPL_FS_LOAD_PAYLOAD_NAME
    err = spl_load_image_fat(spl_image, mmc_get_blk_desc(mmc), CONFIG_SYS_MMCSD_FS_BOOT_PARTITION, filename);
    if (!err)
        return err;
#endif
#endif
    ...
    if (!spl_start_uboot())
    {
        err = spl_load_image_ext_os(spl_image, mmc_get_blk_desc(mmc), CONFIG_SYS_MMCSD_FS_BOOT_PARTITION);
        if (!err)
            return err;
    }
#ifdef CONFIG_SPL_FS_LOAD_PAYLOAD_NAME
    err = spl_load_image_ext(spl_image, mmc_get_blk_desc(mmc), CONFIG_SYS_MMCSD_FS_BOOT_PARTITION, filename);
    if (!err)
        return err;
#endif
#endif
...
#ifdef CONFIG_SYS_MMCSD_RAW_MODE_U_BOOT_USE_SECTOR
        err = mmc_load_image_raw_sector(spl_image, mmc, raw_sect + spl_mmc_raw_uboot_offset(part));
        if (!err)
            return err;
#endif
