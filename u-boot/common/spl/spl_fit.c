                                  ...
                                  const char **outname)
{
    ...
    if (!found && CONFIG_IS_ENABLED(SYSINFO) && !sysinfo_get(&sysinfo))
    {
        ...
        rc = sysinfo_get_fit_loadable(sysinfo, index - i - 1, type, &str);
        if (rc && rc != -ENOENT)
            return rc;
...
static int get_aligned_image_size(struct spl_load_info *info, int data_size, int offset)
{
                              ...
                              struct spl_image_info *image_info)
{
    ...
    else
    {
        ...
        debug("Embedded data: dst=%lx, size=%lx\n", load_addr, (unsigned long)length);
        src = (void *)data; /* cast away const */
    }

    if (CONFIG_IS_ENABLED(FIT_SIGNATURE))
    {
        printf("## Checking hash(es) for Image %s ... ", fit_get_name(fit, node, NULL));
                              ...
                              const struct spl_fit_info *ctx)
{
    ...
    if (node < 0)
    {
         ...
         */
        if (gd->fdt_blob)
            memcpy((void *)image_info.load_addr, gd->fdt_blob, fdt_totalsize(gd->fdt_blob));
        else
            return node;
    }
    else
    ...
    if (CONFIG_IS_ENABLED(LOAD_FIT_APPLY_OVERLAY))
    {
        ...
        for (;; index++)
        {
            ...
            ret = fdt_overlay_apply_verbose(spl_image->fdt_addr, (void *)image_info.load_addr);
            if (ret)
            {
...
static int spl_fit_upload_fpga(struct spl_fit_info *ctx, int node, struct spl_image_info *fpga_image)
{
    ...
    ret = fpga_load(0, (void *)fpga_image->load_addr, fpga_image->size, BIT_FULL);
    if (ret)
    {
...
static int spl_fit_load_fpga(struct spl_fit_info *ctx, struct spl_load_info *info, ulong sector)
{
    int node, ret;
...
static int spl_simple_fit_parse(struct spl_fit_info *ctx)
{
    ...
    {
        printf("## Checking hash(es) for config %s ... ", fit_get_name(ctx->fit, ctx->conf_node, NULL));
                        ...
                        struct spl_load_info *info, ulong sector, void *fit)
{
    ...
    debug("--> spl load simple_fit: read sector %lx \n", sector);

    ret = spl_simple_fit_read(&ctx, info, sector, fit);
    if (ret < 0)
    {
        debug("--> spl spl_simple_fit_read: read err \n");
        return ret;
    }
    ...
    if (ret < 0)
    {
        debug("--> spl err: spl_simple_fit_parse \n");
        return ret;
    }
    ...
    for (;; index++)
    {
         ...
         */
        if (spl_image->entry_point == FDT_ERROR && image_info.entry_point != FDT_ERROR)
            spl_image->entry_point = image_info.entry_point;

        /* Record our loadables into the FDT */
        if (spl_image->fdt_addr)
            spl_fit_record_loadable(&ctx, index, spl_image->fdt_addr, &image_info);
    }
