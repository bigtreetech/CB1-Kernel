...
int spl_parse_legacy_header(struct spl_image_info *spl_image, const struct image_header *header)
{
    ...
    if (IS_ENABLED(CONFIG_SPL_LEGACY_IMAGE_CRC_CHECK) && !image_check_hcrc(header))
    {
                        ...
                        struct spl_load_info *load, ulong header)
{
    ...
    {
    case IH_COMP_NONE:
        load->read(load, dataptr, spl_image->size, (void *)(unsigned long)spl_image->load_addr);
        break;
    ...
    case IH_COMP_LZMA:
        ...
        ret = lzmaBuffToBuffDecompress((void *)spl_image->load_addr, &lzma_len, src, spl_image->size);
        if (ret)
        {
