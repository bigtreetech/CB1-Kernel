...
static int mmc_select_mode(struct mmc *mmc, enum bus_mode mode)
{
    ...
    pr_debug("selecting mode %s (freq : %d MHz)\n", mmc_mode_name(mode), mmc->tran_speed / 1000000);
    return 0;
}
 ...
 */
static int mmc_send_cmd_retry(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data, uint retries)
{
    int ret;
...
int mmc_poll_for_busy(struct mmc *mmc, int timeout_ms)
{
    ...
    while (1)
    {
        ...
        if ((status & MMC_STATUS_RDY_FOR_DATA) && (status & MMC_STATUS_CURR_STATE) != MMC_STATE_PRG)
            break;
...
static int mmc_send_op_cond_iter(struct mmc *mmc, int use_arg)
{
    ...
    if (use_arg && !mmc_host_is_spi(mmc))
        cmd.cmdarg = OCR_HCS | (mmc->cfg->voltages & (mmc->ocr & OCR_VOLTAGE_MASK)) | (mmc->ocr & OCR_ACCESS_MODE);
...
static int __mmc_switch(struct mmc *mmc, u8 set, u8 index, u8 value, bool send_status)
{
    unsigned int status, start;
    struct mmc_cmd cmd;
    int timeout_ms = DEFAULT_CMD6_TIMEOUT_MS;
    bool is_part_switch = (set == EXT_CSD_CMD_SET_NORMAL) && (index == EXT_CSD_PART_CONF);
    int ret;
...
#if CONFIG_IS_ENABLED(MMC_IO_VOLTAGE)
static int mmc_set_lowest_voltage(struct mmc *mmc, enum bus_mode mode, uint32_t allowed_mask)
{
    u32 card_mask = 0;
    ...
    case MMC_HS_200:
        ...
        if (mmc->cardtype & (EXT_CSD_CARD_TYPE_HS200_1_2V | EXT_CSD_CARD_TYPE_HS400_1_2V))
            card_mask |= MMC_SIGNAL_VOLTAGE_120;
        break;
    case MMC_DDR_52:
        if (mmc->cardtype & EXT_CSD_CARD_TYPE_DDR_1_8V)
            card_mask |= MMC_SIGNAL_VOLTAGE_330 | MMC_SIGNAL_VOLTAGE_180;
...
#endif
#if CONFIG_IS_ENABLED(MMC_HW_PARTITIONING)
    mmc->hc_wp_grp_size = 1024 * ext_csd[EXT_CSD_HC_ERASE_GRP_SIZE] * ext_csd[EXT_CSD_HC_WP_GRP_SIZE];
#endif
...
int mmc_get_op_cond(struct mmc *mmc, bool quiet)
{
    ...
    mmc->quirks = MMC_QUIRK_RETRY_SET_BLOCKLEN | MMC_QUIRK_RETRY_SEND_CID | MMC_QUIRK_RETRY_APP_CMD;
#endif
    ...
    if (IS_ENABLED(CONFIG_MMC_SPEED_MODE_SET))
    {
        if (mmc->user_speed_mode != MMC_MODES_END)
        {
            ...
            if (mmc->host_caps & MMC_CAP(mmc->user_speed_mode))
            {
                ...
                mmc->host_caps |= (MMC_CAP(mmc->user_speed_mode) | MMC_CAP(MMC_LEGACY) |
                                   MMC_MODE_1BIT);
            }
            else
            {
...
int mmc_deinit(struct mmc *mmc)
{
    ...
    {
        caps_filtered = mmc->card_caps & ~(MMC_CAP(MMC_HS_200) | MMC_CAP(MMC_HS_400) | MMC_CAP(MMC_HS_400_ES));
