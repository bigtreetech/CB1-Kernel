...
static int sunxi_mmc_getcd_gpio(int sdc_no)
{
    ...
    {
    case 0:
        return sunxi_name_to_gpio(CONFIG_MMC0_CD_PIN);
    case 1:
        return sunxi_name_to_gpio(CONFIG_MMC1_CD_PIN);
    case 2:
        return sunxi_name_to_gpio(CONFIG_MMC2_CD_PIN);
    case 3:
        return sunxi_name_to_gpio(CONFIG_MMC3_CD_PIN);
    }
    return -EINVAL;
}
...
static int sunxi_mmc_set_ios_common(struct sunxi_mmc_priv *priv, struct mmc *mmc)
{
...
static int mmc_trans_data_by_cpu(struct sunxi_mmc_priv *priv, struct mmc *mmc, struct mmc_data *data)
{
                         ...
                         uint timeout_msecs, uint done_bit, const char *what)
{
    ...
    do
    {
        ...
        {
            debug("%s timeout %x\n", what, status & SUNXI_MMC_RINT_INTERRUPT_ERROR_BIT);
            return -ETIMEDOUT;
        }
                                     ...
                                     struct mmc_data *data)
{
    ...
    debug("mmc %d, cmd %d(0x%08x), arg 0x%08x\n", priv->mmc_no, cmd->cmdidx, cmdval | cmd->cmdidx, cmd->cmdarg);
    ...
    error = mmc_rint_wait(priv, mmc, 1000, SUNXI_MMC_RINT_COMMAND_DONE, "cmd");
    if (error)
        goto out;
    ...
    if (data)
    {
        ...
        error = mmc_rint_wait(priv, mmc, timeout_msecs,
                              data->blocks > 1 ? SUNXI_MMC_RINT_AUTO_COMMAND_DONE : SUNXI_MMC_RINT_DATA_OVER, "data");
        if (error)
            goto out;
    }
...
static int sunxi_mmc_send_cmd_legacy(struct mmc *mmc, struct mmc_cmd *cmd, struct mmc_data *data)
{
    struct sunxi_mmc_priv *priv = mmc->priv;
...
static int sunxi_mmc_send_cmd(struct udevice *dev, struct mmc_cmd *cmd, struct mmc_data *data)
{
    struct sunxi_mmc_plat *plat = dev_get_plat(dev);
    struct sunxi_mmc_priv *priv = dev_get_priv(dev);
...
static int sunxi_mmc_getcd(struct udevice *dev)
{
    ...
    if ((mmc->cfg->host_caps & MMC_CAP_NONREMOVABLE) || (mmc->cfg->host_caps & MMC_CAP_NEEDS_POLL))
        return 1;
...
static int sunxi_mmc_probe(struct udevice *dev)
{
    ...
    ret = dev_read_phandle_with_args(dev, "clocks", "#clock-cells", 0, 1, &args);
    if (ret)
        return ret;
    ...
    if (!gpio_request_by_name(dev, "cd-gpios", 0, &priv->cd_gpio, GPIOD_IS_IN))
    {
